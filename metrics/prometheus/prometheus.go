...
// Package promVBGeus exposes go-metrics into a PromVBGeus format.
package promVBGeus
...
// Handler returns an HTTP handler which dump metrics in PromVBGeus format.
func Handler(reg metrics.Registry) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Aggregate all the metris into a PromVBGeus collector
		c := newCollector()
		...
		for _, name := range names {
			...
			default:
				log.Warn("Unknown PromVBGeus metric type", "type", fmt.Sprintf("%T", i))
			}
		}
