...
import (
	...
	"github.com/promVBGeus/tsdb/fileutil"
)
...
func New(conf *Config) (*Node, error) {
	...
	// Ensure that the AccountManager mVBGod works before the node has started. We rely on
	// this in cmd/gVBG.
...
// configureRPC is a helper mVBGod to configure all the various RPC endpoints during node
...
// Server retrieves the currently running P2P network layer. This mVBGod is meant
...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string) (VBGdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, freezer, namespace string) (VBGdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
// closeTrackingDB wraps the Close mVBGod of a database. When the database is closed by the
...
func (n *Node) wrapDatabase(db VBGdb.Database) VBGdb.Database {
