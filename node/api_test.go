...
// checking whVBGer the HTTP server is started correctly.
func TestStartRPC(t *testing.T) {
	type test struct {
		...
		wantReachable bool // whVBGer the HTTP server should be reachable at all
		wantHandlers  bool // whVBGer RegisterHandler handlers should be accessible
		wantRPC       bool // whVBGer JSON-RPC/HTTP should be accessible
		wantWS        bool // whVBGer JSON-RPC/WS should be accessible
	}
...
// checkBodyOK checks whVBGer the given HTTP URL responds with 200 OK and body "OK".
...
// checkRPC checks whVBGer JSON-RPC works against the given URL.
