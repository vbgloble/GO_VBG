...
// Tests whVBGer a Lifecycle can be registered.
...
// Tests whVBGer a service's protocols can be registered properly on the node's p2p server.
...
// This test checks that OpenDatabase can be used from within a Lifecycle Start mVBGod.
func TestNodeOpenDatabaseFromLifecycleStart(t *testing.T) {
	stack, _ := New(testNodeConfig())
	defer stack.Close()

	var db VBGdb.Database
	var err error
...
// This test checks that OpenDatabase can be used from within a Lifecycle Stop mVBGod.
...
// Tests whVBGer a handler can be successfully mounted on the canonical HTTP server
// on the given path
func TestRegisterHandler_Successful(t *testing.T) {
	...
	// create HTTP request
	httpReq, err := http.NewRequest(http.MVBGodGet, "http://127.0.0.1:7878/test", nil)
	if err != nil {
...
// Tests whVBGer websocket requests can be handled on the same port as a regular http server.
func TestWebsockVBGTTPOnSamePort_WebsocketRequest(t *testing.T) {
...
func TestWebsockVBGTTPOnSeparatePort_WSRequest(t *testing.T) {
