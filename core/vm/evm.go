...
type (
	...
	// GVBGashFunc returns the n'th block hash in the blockchain
	// and is used by the BLOCKHASH EVM op code.
	GVBGashFunc func(uint64) common.Hash
)
...
type BlockContext struct {
	// CanTransfer returns whVBGer the account contains
	// sufficient VBGer to transfer the value
	CanTransfer CanTransferFunc
	// Transfer transfers VBGer from one account to the other
	Transfer TransferFunc
	// GVBGash returns the hash corresponding to n
	GVBGash GVBGashFunc
...
// EVM is the vbgloble Virtual Machine base object and provides
...
type EVM struct {
	...
	// evm.
	vmConfig Config
	// global (to this context) vbgloble virtual machine
...
func (evm *EVM) CallCode(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	// Fail if we're trying to transfer more than the available balance
	// Note although it's noop to transfer X VBGer to caller itself. But
...
func (evm *EVM) create(caller ContractRef, codeAndHash *codeAndHash, gas uint64, value *big.Int, address common.Address) ([]byte, common.Address, uint64, error) {
	...
	// check whVBGer the max code size has been exceeded
