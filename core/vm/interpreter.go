...
// Interpreter is used to run vbgloble based contracts and will utilise the
...
// keccakState wraps sha3.state. In addition to the usual hash mVBGods, it also supports
...
type EVMInterpreter struct {
	...
	readOnly   bool   // WhVBGer to throw on stateful modifications
	returnData []byte // Last CALL's return data for subsequent reuse
}

// NewEVMInterpreter returns a new instance of the Interpreter.
func NewEVMInterpreter(evm *EVM, cfg Config) *EVMInterpreter {
	// We use the STOP instruction whVBGer to see
...
func (in *EVMInterpreter) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, err error) {
	...
	// Don't move this deferrred function, it's placed before the capturestate-deferred mVBGod,
	...
	for {
		...
		// cost is explicitly set so that the capture state defer mVBGod can get the proper cost
