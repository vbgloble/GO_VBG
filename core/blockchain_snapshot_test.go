...
// (i)   GVBG restarts normally with valid legacy snapshot
// (ii)  GVBG restarts normally with valid new-format snapshot
// (iii) GVBG restarts after the crash, with broken legacy snapshot
// (iv)  GVBG restarts after the crash, with broken new-format snapshot
// (v)   GVBG restarts normally, but it's requested to be rewound to a lower point via SVBGead
// (vi)  GVBG restarts normally with a stale snapshot
type snapshotTest struct {
	legacy       bool   // Flag whVBGer the loaded snapshot is in legacy format
	crash        bool   // Flag whVBGer the GVBG restarts from the previous crash
	...
	sVBGead      uint64 // Block number to set head back to
...
// Tests a GVBG restart with valid snapshot. Before the shutdown, all snapshot
...
// Tests a GVBG restart with valid but "legacy" snapshot. Before the shutdown,
...
// Tests a GVBG was crashed and restarts with a broken snapshot. In this case the
...
// Tests a GVBG was crashed and restarts with a broken snapshot. In this case
...
// Tests a GVBG was crashed and restarts with a broken and "legacy format"
...
// Tests a GVBG was running with snapshot enabled. Then restarts without
...
// Tests a GVBG was running with leagcy snapshot enabled. Then restarts
...
// Tests the GVBG was running with snapshot enabled and resVBGead is applied.
...
func TestSVBGeadWithNewSnapshot(t *testing.T) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8 (HEAD)
	//
	// Commit:   G
...
// Tests the GVBG was running with snapshot(legacy-format) enabled and resVBGead
...
func TestSVBGeadWithLegacySnapshot(t *testing.T) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8 (HEAD)
	//
	// Commit:   G
...
// Tests the GVBG was running with snapshot(legacy-format) enabled and upgrades
// the disk layer journal(journal generator) to latest format. After that the GVBG
// is restarted from a crash. In this case GVBG will find the new-format disk layer
...
func testSnapshot(t *testing.T, tt *snapshotTest) {
	...
	chain, err := NewBlockChain(db, cacheConfig, params.AllVBGashProtocolChanges, engine, vm.Config{}, nil, nil)
	if err != nil {
	...
	if tt.crash {
		...
		chain, err = NewBlockChain(db, nil, params.AllVBGashProtocolChanges, engine, vm.Config{}, nil, nil)
		if err != nil {
		...
		defer chain.Stop()
	} else if tt.sVBGead != 0 {
		// Rewind the chain if sVBGead operation is required.
		chain.SVBGead(tt.sVBGead)
		chain.Stop()
