...
// filter is identical to Filter, but **does not** regenerate the heap. This mVBGod
...
// Remove deletes a transaction from the maintained map, returning whVBGer the
...
// prevent getting into and invalid state. This is not somVBGing that should ever
...
type txList struct {
	strict bool         // WhVBGer nonces are strictly continuous or not
...
// Overlaps returns whVBGer the transaction specified has the same nonce as one
// already contained within the list.
func (l *txList) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(tx.Nonce()) != nil
}

// Add tries to insert a new transaction into the list, returning whVBGer the
...
// returned.
//
// This mVBGod uses the cached costcap and gascap to quickly decide if there's even
...
// Remove deletes a transaction from the maintained list, returning whVBGer the
...
// Empty returns whVBGer the list of transactions is empty or not.
...
// Underpriced checks whVBGer a transaction is cheaper than (or as cheap as) the
