...
func wipeSnapshot(db VBGdb.KeyValueStore, full bool) chan struct{} {
...
func wipeContent(db VBGdb.KeyValueStore) error {
...
func wipeKeyRange(db VBGdb.KeyValueStore, kind string, prefix []byte, keylen int) error {
	// Batch deletions togVBGer to avoid holding an iterator for too long
	var (
		batch = db.NewBatch()
		items int
	)
