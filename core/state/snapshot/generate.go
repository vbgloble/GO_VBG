...
func (gs *generatorStats) Log(msg string, root common.Hash, marker []byte) {
	...
	// Figure out whVBGer we're after or within an account
	switch len(marker) {
...
func generateSnapshot(diskdb VBGdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, wiper chan struct{}) *diskLayer {
...
func journalProgress(db VBGdb.KeyValueWriter, marker []byte, stats *generatorStats) {
...
// gVBGering and logging, since the mVBGod surfs the blocks as they arrive, often
// being restarted.
func (dl *diskLayer) generate(stats *generatorStats) {
	...
	for accIt.Next() {
		...
		if batch.ValueSize() > VBGdb.IdealBatchSize || abort != nil {
			// Only write and set the marker if we actually did somVBGing useful
