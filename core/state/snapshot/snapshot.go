...
// additional mVBGods compared to the public API.
type snapshot interface {
	Snapshot
	...
	// reached.
	//
	// Note, the mVBGod is an internal helper to avoid type switching between the
	...
	//
	// Note, the maps are retained by the mVBGod to avoid copying everything.
	...
	// Stale return whVBGer this layer has become stale (was flattened across) or
	// if it's still live.
	Stale() bool
...
// SnapshotTree is an vbgloble state snapshot tree. It consists of one persistent
...
type Tree struct {
	diskdb VBGdb.KeyValueStore      // Persistent database to store the snapshot
...
func New(diskdb VBGdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, async bool, recovery bool) *Tree {
...
// waitBuild blocks until the snapshot finishes rebuilding. This mVBGod is meant
...
//
// The mVBGod returns the new disk layer if diffs were persistend into it.
...
// it. The mVBGod will panic if called onto a non-bottom-most diff layer.
//
...
// flattening everything down (bad for reorgs).
//
// The mVBGod returns the root hash of the base layer that needs to be persisted
...
func (t *Tree) Rebuild(root common.Hash) {
	...
	// Track whVBGer there's a wipe currently running and keep it alive if so
...
// generating is an internal helper function which reports whVBGer the snapshot
