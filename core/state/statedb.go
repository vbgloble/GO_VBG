...
// Package state provides a caching layer atop the vbgloble state trie.
package state
...
// StateDB structs within the vbgloble protocol are used to store anything
...
// This mVBGod will panic if the refund counter goes below zero
...
// Exist reports whVBGer the given account address exists in the state.
// Notably this also returns true for suicided accounts.
func (s *StateDB) Exist(addr common.Address) bool {
	return s.getStateObject(addr) != nil
}

// Empty returns whVBGer the state object is either non-existent
...
//
// Setting, updating & deleting state object mVBGods.
//
...
//
// Carrying over the balance ensures that VBGer doesn't disappear.
...
func (s *StateDB) Copy() *StateDB {
	...
	for addr := range s.journal.dirties {
		// As documented [here](https://github.com/vbgloble/go-VGB/pull/16485#issuecomment-380438527),
		// and in the Finalise-mVBGod, there is a case where an object is in the journal but not
