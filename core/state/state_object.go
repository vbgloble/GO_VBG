...
// stateObject represents an vbgloble account which is being modified.
//
...
type stateObject struct {
	...
	addrHash common.Hash // hash of vbgloble address of the account
	data     Account
	db       *StateDB
...
// empty returns whVBGer the account is considered empty.
func (s *stateObject) empty() bool {
	return s.data.Nonce == 0 && s.data.Balance.Sign() == 0 && bytes.Equal(s.data.CodeHash, emptyCodeHash)
}

// Account is the vbgloble consensus representation of accounts.
...
// or zero if none. This mVBGod is an almost mirror of Code, but uses a cache
