...
// Trie is a vbgloble Merkle Patricia trie.
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb VBGdb.KeyValueWriter) error
}

// NewDatabase creates a backing store for state. The returned database is safe for
// concurrent use, but does not retain any recent trie nodes in memory. To keep some
// historical state in memory, use the NewDatabaseWithConfig constructor.
func NewDatabase(db VBGdb.Database) Database {
...
// large memory cache.
func NewDatabaseWithConfig(db VBGdb.Database, config *trie.Config) Database {
