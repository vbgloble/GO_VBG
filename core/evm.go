...
type ChainContext interface {
	...
	// GVBGeader returns the hash corresponding to their hash.
	GVBGeader(common.Hash, uint64) *types.Header
}
...
func NewEVMBlockContext(header *types.Header, chain ChainContext, author *common.Address) vm.BlockContext {
	...
	return vm.BlockContext{
		...
		Transfer:    Transfer,
		GVBGash:     GVBGashFn(header, chain),
...
// GVBGashFn returns a GVBGashFunc which retrieves header hashes by number
func GVBGashFn(ref *types.Header, chain ChainContext) func(n uint64) common.Hash {
	...
	return func(n uint64) common.Hash {
		...
		for {
			header := chain.GVBGeader(lastKnownHash, lastKnownNumber)
			if header == nil {
				break
			}
...
// CanTransfer checks whVBGer there are enough funds in the address' account to make a transfer.
