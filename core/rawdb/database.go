...
import (
	...
	"github.com/vbgloble/go-VGB/VBGdb/leveldb"
	"github.com/vbgloble/go-VGB/VBGdb/memorydb"
...
// Freeze is a helper mVBGod used for external testing to trigger and block until
...
func NewDatabase(db VBGdb.KeyValueStore) VBGdb.Database {
	return &nofreezedb{
		KeyValueStore: db,
	}
}
...
// storage.
func NewDatabaseWithFreezer(db VBGdb.KeyValueStore, freezer string, namespace string) (VBGdb.Database, error) {
	...
	// validate in this mVBGod. If, however, the genesis hash is not nil, compare
	// it to the freezer content.
	if kvgenesis, _ := db.Get(headerHashKey(0)); len(kvgenesis) > 0 {
		if frozen, _ := frdb.Ancients(); frozen > 0 {
			// If the freezer already contains somVBGing, ensure that the genesis blocks
...
func NewMemoryDatabase() VBGdb.Database {
...
func NewMemoryDatabaseWithCap(size int) VBGdb.Database {
	return NewDatabase(memorydb.NewWithCap(size))
}

// NewLevelDBDatabase creates a persistent key-value database without a freezer
// moving immutable chain segments into cold storage.
func NewLevelDBDatabase(file string, cache int, handles int, namespace string) (VBGdb.Database, error) {
...
// freezer moving immutable chain segments into cold storage.
func NewLevelDBDatabaseWithFreezer(file string, cache int, handles int, freezer string, namespace string) (VBGdb.Database, error) {
...
func InspectDatabase(db VBGdb.Database) error {
	...
	table.SVBGeader([]string{"Database", "Category", "Size", "Items"})
