...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for VBGstats availability")
	if infos, err := checkVBGstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["VBGstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["VBGstats"] = infos.Report()
		VBGstats = infos.config
	}
...
func (stats serverStats) render() {
	...
	table.SVBGeader([]string{"Server", "Address", "Service", "Config", "Value"})
