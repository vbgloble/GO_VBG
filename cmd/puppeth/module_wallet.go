...
FROM puppVBG/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gVBG --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gVBG --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --VBGstats \'{{.VBGstats}}\' --cache=512 --http --http.addr=0.0.0.0 --http.corsdomain "*" --http.vhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppVBGNetworkID/{{.NetworkID}}/g' dist/js/VBGerwallet-master.js && \
	sed -i 's/PuppVBGNetwork/{{.Network}}/g'     dist/js/VBGerwallet-master.js && \
	sed -i 's/PuppVBGDenom/{{.Denom}}/g'         dist/js/VBGerwallet-master.js && \
	sed -i 's/PuppVBGHost/{{.Host}}/g'           dist/js/VBGerwallet-master.js && \
	sed -i 's/PuppVBGRPCPort/{{.RPCPort}}/g'     dist/js/VBGerwallet-master.js
...
services:
  wallet:
    ...
    ports:
      ...
      - "{{.WebPort}}:80"{{end}}
    volumes:
      - {{.Datadir}}:/root/.vbgloble
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.VBGstats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
      - VIRTUAL_PORT=80{{end}}
    logging:
      driver: "json-file"
...
// checkWallet does a health-check against web wallet server to verify whVBGer
// it's running, and if yes, whVBGer it's responsive.
