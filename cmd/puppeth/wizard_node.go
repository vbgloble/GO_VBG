...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.VBGstats == "" {
		log.Error("No VBGstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.VBGash != nil && !boot {
		fmt.Println()
		if infos.VBGashdir == "" {
			fmt.Printf("Where should the VBGash mining DAGs be stored on the remote machine?\n")
			infos.VBGashdir = w.readString()
		} else {
			fmt.Printf("Where should the VBGash mining DAGs be stored on the remote machine? (default = %s)\n", infos.VBGashdir)
			infos.VBGashdir = w.readDefaultString(infos.VBGashdir)
		}
	}
	...
	if infos.VBGstats == "" {
		...
		infos.VBGstats = w.readString() + ":" + w.conf.VBGstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.VBGstats)
		infos.VBGstats = w.readDefaultString(infos.VBGstats) + ":" + w.conf.VBGstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.VBGash != nil {
			// VBGash based miners only need an VBGerbase to mine against
			fmt.Println()
			if infos.VBGerbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.VBGerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.VBGerbase)
				infos.VBGerbase = w.readDefaultAddress(common.HexToAddress(infos.VBGerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy vbgloble node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
