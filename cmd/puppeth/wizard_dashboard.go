...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"VBGstats", "explorer", "wallet", "faucet"} {
	...
	// If we have VBGstats running, ask whVBGer to make the secret public or not
	if w.conf.VBGstats != "" {
		fmt.Println()
		fmt.Println("Include VBGstats secret on dashboard (y/n)? (default = yes)")
