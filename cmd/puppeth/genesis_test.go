...
// Tests the go-VGB to AlVBG chainspec conversion for the Stureby testnet.
func TestAlVBGSturebyConverter(t *testing.T) {
	blob, err := ioutil.ReadFile("testdata/stureby_gVBG.json")
	if err != nil {
	...
	spec, err := newAlVBGGenesisSpec("stureby", &genesis)
	if err != nil {
		t.Fatalf("failed creating chainspec: %v", err)
	}

	expBlob, err := ioutil.ReadFile("testdata/stureby_alVBG.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	expspec := &alVBGGenesisSpec{}
...
// Tests the go-VGB to Parity chainspec conversion for the Stureby testnet.
