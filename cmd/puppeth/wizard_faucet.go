...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many VBGers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.VBGstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.VBGstats = w.readString() + ":" + w.conf.VBGstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.VBGstats)
		infos.node.VBGstats = w.readDefaultString(infos.node.VBGstats) + ":" + w.conf.VBGstats
	}
