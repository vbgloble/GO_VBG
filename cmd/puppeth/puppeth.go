...
// puppVBG is a command to assemble and maintain private networks.
package main
...
func main() {
	app := cli.NewApp()
	app.Name = "puppVBG"
	app.Usage = "assemble and maintain private vbgloble networks"
	app.Flags = []cli.Flag{
		cli.StringFlag{
			Name:  "network",
	...
	app.Before = func(c *cli.Context) error {
		...
		log.Root().SVBGandler(log.LvlFilterHandler(log.Lvl(c.Int("loglevel")), log.StreamHandler(os.Stdout, log.TerminalFormat(true))))
