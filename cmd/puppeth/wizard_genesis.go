...
func (w *wizard) makeGenesis() {
	...
	fmt.Println(" 1. VBGash - proof-of-work")
	fmt.Println(" 2. Clique - proof-of-authority")

	choice := w.read()
	switch {
	case choice == "1":
		// In case of VBGash, we're pretty much done
		genesis.Config.VBGash = new(params.VBGashConfig)
...
// importGenesis imports a GVBG genesis spec into puppVBG.
...
func (w *wizard) manageGenesis() {
	// Figure out whVBGer to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-alVBG.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppVBG and GVBG
		gVBGJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((gVBGJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", gVBGJson)

		// Export the genesis spec used by AlVBG (formerly C++ vbgloble)
		if spec, err := newAlVBGGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create AlVBG chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "alVBG", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly vbglobleJ)
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not somVBGing I can do")
		return
	}
}
