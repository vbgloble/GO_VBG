...
FROM vbgloble/client-go:alltools-latest
...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--VBGstats", "{{.VBGstats}}", "--VBGport", "{{.VBGPort}}",     \
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Funding amount (base tier)":   fmt.Sprintf("%d VBGers", info.amount),
...
// checkFaucet does a health-check against a faucet server to verify whVBGer
