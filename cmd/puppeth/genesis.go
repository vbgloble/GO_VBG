...
import (
	...
	math2 "github.com/vbgloble/go-VGB/common/math"
...
// alVBGGenesisSpec represents the genesis specification format used by the
// C++ vbgloble implementation.
type alVBGGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*alVBGGenesisSpecAccount `json:"accounts"`
}

// alVBGGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type alVBGGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256   `json:"balance,omitempty"`
	Nonce       uint64                   `json:"nonce,omitempty"`
	Precompiled *alVBGGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// alVBGGenesisSpecBuiltin is the precompiled contract definition.
type alVBGGenesisSpecBuiltin struct {
	Name          string                         `json:"name,omitempty"`
	StartingBlock *hexutil.Big                   `json:"startingBlock,omitempty"`
	Linear        *alVBGGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type alVBGGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newAlVBGGenesisSpec converts a go-VGB genesis block into a AlVBG-specific
// chain specification format.
func newAlVBGGenesisSpec(network string, genesis *core.Genesis) (*alVBGGenesisSpec, error) {
	// Only VBGash is currently supported between go-VGB and alVBG
	if genesis.Config.VBGash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in AlVBG format
	spec := &alVBGGenesisSpec{
		SealEngine: "VBGash",
	}
	// Some defaults
	...
	// config but alVBG will sync with ETC clients up until the actual dao hard
	// fork block.
	...
	spec.Params.BlockReward = (*hexutil.Big)(VBGash.FrontierBlockReward)
	...
	spec.setPrecompile(1, &alVBGGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &alVBGGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &alVBGGenesisSpecBuiltin{Name: "sha256",
		Linear: &alVBGGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &alVBGGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &alVBGGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &alVBGGenesisSpecBuiltin{Name: "identity",
		Linear: &alVBGGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &alVBGGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
		spec.setPrecompile(6, &alVBGGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &alVBGGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &alVBGGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &alVBGGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &alVBGGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
	}
	if genesis.Config.IstanbulBlock != nil {
		...
		spec.setPrecompile(6, &alVBGGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // AlVBG hardcoded the gas policy
		spec.setPrecompile(7, &alVBGGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // AlVBG hardcoded the gas policy
		spec.setPrecompile(9, &alVBGGenesisSpecBuiltin{
...
func (spec *alVBGGenesisSpec) setPrecompile(address byte, data *alVBGGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alVBGGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &alVBGGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *alVBGGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alVBGGenesisSpecAccount)
	}
...
type parityChainSpecPricing struct {
	...
	// Before the https://github.com/paritytech/parity-vbgloble/pull/11039,
...
// newParityChainSpec converts a go-VGB genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only VBGash is currently supported between go-VGB and Parity
	if genesis.Config.VBGash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	spec.Engine.VBGash.Params.BlockReward = make(map[string]string)
	spec.Engine.VBGash.Params.DifficultyBombDelays = make(map[string]string)
	// Frontier
	spec.Engine.VBGash.Params.MinimumDifficulty = (*hexutil.Big)(params.MinimumDifficulty)
	spec.Engine.VBGash.Params.DifficultyBoundDivisor = (*hexutil.Big)(params.DifficultyBoundDivisor)
	spec.Engine.VBGash.Params.DurationLimit = (*hexutil.Big)(params.DurationLimit)
	spec.Engine.VBGash.Params.BlockReward["0x0"] = hexutil.EncodeBig(VBGash.FrontierBlockReward)

	// Homestead
	spec.Engine.VBGash.Params.HomesteadTransition = hexutil.Uint64(genesis.Config.HomesteadBlock.Uint64())

	// Tangerine Whistle : 150
	// https://github.com/vbgloble/EIPs/blob/master/EIPS/eip-608.md
	spec.Params.EIP150Transition = hexutil.Uint64(genesis.Config.EIP150Block.Uint64())

	// Spurious Dragon: 155, 160, 161, 170
	// https://github.com/vbgloble/EIPs/blob/master/EIPS/eip-607.md
	...
	// gVBG has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.vbgloble.Nonce = types.EncodeNonce(genesis.Nonce)
	spec.Genesis.Seal.vbgloble.MixHash = (genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
func (spec *parityChainSpec) setByzantium(num *big.Int) {
	spec.Engine.VBGash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(VBGash.ByzantiumBlockReward)
	spec.Engine.VBGash.Params.DifficultyBombDelays[hexutil.EncodeBig(num)] = hexutil.EncodeUint64(3000000)
	n := hexutil.Uint64(num.Uint64())
	spec.Engine.VBGash.Params.EIP100bTransition = n
...
func (spec *parityChainSpec) setConstantinople(num *big.Int) {
	spec.Engine.VBGash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(VBGash.ConstantinopleBlockReward)
	spec.Engine.VBGash.Params.DifficultyBombDelays[hexutil.EncodeBig(num)] = hexutil.EncodeUint64(2000000)
	n := hexutil.Uint64(num.Uint64())
...
// pyvbglobleGenesisSpec represents the genesis specification format used by the
// Python vbgloble implementation.
type pyvbglobleGenesisSpec struct {
	Nonce      types.BlockNonce  `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyvbglobleGenesisSpec converts a go-VGB genesis block into a Parity specific
// chain specification format.
func newPyvbglobleGenesisSpec(network string, genesis *core.Genesis) (*pyvbglobleGenesisSpec, error) {
	// Only VBGash is currently supported between go-VGB and pyvbgloble
	if genesis.Config.VBGash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyvbglobleGenesisSpec{
