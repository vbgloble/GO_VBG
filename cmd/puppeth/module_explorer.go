...
FROM puppVBG/blockscout:latest

ADD genesis.json /genesis.json
RUN \
  echo 'gVBG --cache 512 init /genesis.json' > explorer.sh && \
  echo $'gVBG --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.VBGPort}} --bootnodes {{.Bootnodes}} --VBGstats \'{{.VBGstats}}\' --cache=512 --http --http.api "net,web3,VBG,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec gVBG --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.VBGPort}} --bootnodes {{.Bootnodes}} --VBGstats \'{{.VBGstats}}\' --cache=512 --http --http.api "net,web3,VBG,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" &' >> explorer.sh && \
...
services:
    explorer:
        ...
        ports:
            - "{{.VBGPort}}:{{.VBGPort}}"
            - "{{.VBGPort}}:{{.VBGPort}}/udp"{{if not .VHost}}
            - "{{.WebPort}}:4000"{{end}}
        environment:
            - VBG_PORT={{.VBGPort}}
            - VBG_NAME={{.VBGName}}
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"VBGstats":  config.node.VBGstats,
		"VBGPort":   config.node.port,
	})
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"VBGPort":     config.node.port,
		"VBGName":     config.node.VBGstats[:strings.Index(config.node.VBGstats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"vbgloble listener port ": strconv.Itoa(info.node.port),
		"VBGstats username":       info.node.VBGstats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	p2pPort := infos.portmap[infos.envvars["VBG_PORT"]+"/tcp"]
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.vbgloble"],
			port:     infos.portmap[infos.envvars["VBG_PORT"]+"/tcp"],
			VBGstats: infos.envvars["VBG_NAME"],
		},
