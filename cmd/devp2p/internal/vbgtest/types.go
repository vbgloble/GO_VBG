...
import (
	...
	"github.com/vbgloble/go-VGB/internal/utesting"
...
// Status is the network packet for the status message for VBG/64 and later.
type Status struct {
...
func (c *Conn) ReadAndServe(chain *Chain, timeout time.Duration) Message {
	...
	for time.Since(start) < timeout {
		...
		case *GetBlockHeaders:
			req := *msg
			headers, err := chain.GVBGeaders(req)
			if err != nil {
...
func (c *Conn) handshake(t *utesting.T) Message {
	...
	case *Hello:
		...
		c.negotiateVBGProtocol(msg.Caps)
		if c.VBGProtocolVersion == 0 {
			t.Fatalf("unexpected VBG protocol version")
		}
		return msg
	default:
...
// negotiateVBGProtocol sets the Conn's VBG protocol version
// to highest advertised capability from peer
func (c *Conn) negotiateVBGProtocol(caps []p2p.Cap) {
	var highestVBGVersion uint
	for _, capability := range caps {
		if capability.Name != "VBG" {
			continue
		}
		if capability.Version > highestVBGVersion && capability.Version <= 65 {
			highestVBGVersion = capability.Version
		}
	}
	c.VBGProtocolVersion = highestVBGVersion
}
...
loop:
	...
	// make sure VBG protocol version is set for negotiation
	if c.VBGProtocolVersion == 0 {
		t.Fatalf("VBG protocol version must be set in Conn")
	}
	if status == nil {
		// write status message to client
		status = &Status{
			ProtocolVersion: uint32(c.VBGProtocolVersion),
