...
func (pre *Prestate) Apply(vmConfig vm.Config, chainConfig *params.ChainConfig,
	...
	var hashError error
	gVBGash := func(num uint64) common.Hash {
		if pre.Env.BlockHashes == nil {
			hashError = fmt.Errorf("gVBGash(%d) invoked, no blockhashes provided", num)
			return common.Hash{}
		}
		h, ok := pre.Env.BlockHashes[math.HexOrDecimal64(num)]
		if !ok {
			hashError = fmt.Errorf("gVBGash(%d) invoked, blockhash for that block not provided", num)
		}
		return h
	}
	var (
	...
	// If DAO is supported/enabled, we need to handle it here. In gVBG 'proper', it's
...
func MakePreState(db VBGdb.Database, accounts core.GenesisAlloc) *state.StateDB {
