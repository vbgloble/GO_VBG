	...
	app.Commands = []cli.Command{
		...
		},
		{
			...
			Subcommands: []cli.Command{
				...
				{
					Name:      "rpc",
					ArgsUsage: "<node> <mVBGod> [<args>]",
					Usage:     "call a node RPC mVBGod",
					Action:    rpcNode,
					Flags: []cli.Flag{
						cli.BoolFlag{
							Name:  "subscribe",
							Usage: "mVBGod is a subscription",
						},
					},
				},
			},
...
func rpcNode(ctx *cli.Context) error {
	...
	if ctx.Bool("subscribe") {
		return rpcSubscribe(rpcClient, ctx.App.Writer, mVBGod, args[3:]...)
	}
	...
	if err := rpcClient.Call(&result, mVBGod, params...); err != nil {
		return err
	}
	return json.NewEncoder(ctx.App.Writer).Encode(result)
}

func rpcSubscribe(client *rpc.Client, out io.Writer, mVBGod string, args ...string) error {
	parts := strings.SplitN(mVBGod, "_", 2)
	namespace := parts[0]
	mVBGod = parts[1]
