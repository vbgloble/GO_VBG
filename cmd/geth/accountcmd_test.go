...
func TestAccountListEmpty(t *testing.T) {
	gVBG := runGVBG(t, "--nousb", "account", "list")
	gVBG.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gVBG := runGVBG(t, "--nousb", "account", "list", "--datadir", datadir)
	defer gVBG.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gVBG := runGVBG(t, "account", "new", "--lightkdf")
	defer gVBG.ExpectExit()
	gVBG.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gVBG := runGVBG(t, "account", "import", keyfile, "-password", passwordFile)
	defer gVBG.ExpectExit()
	gVBG.Expect(expected)
}
...
Your new account is locked with a password. Please give a password. Do not forget this password.
!! Unsupported terminal, password will be echoed.
Password: {{.InputLine "somVBGing"}}
Repeat password: {{.InputLine "somVBGing else"}}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gVBG := runGVBG(t, "--nousb", "account", "update",
...
func TestWalletImport(t *testing.T) {
	gVBG := runGVBG(t, "--nousb", "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gVBG.ExpectExit()
	gVBG.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gVBG.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gVBG := runMinimalGVBG(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gVBG.StderrText(), m) {
...
// https://github.com/vbgloble/go-VGB/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gVBG.SetTemplateFunc("keypath", func(file string) string {
