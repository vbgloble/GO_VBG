...
func (g *gVBGrpc) killAndWait() {
	g.gVBG.Kill()
	g.gVBG.WaitExit()
}

func (g *gVBGrpc) callRPC(result interface{}, mVBGod string, args ...interface{}) {
	if err := g.rpc.Call(&result, mVBGod, args...); err != nil {
		g.gVBG.Fatalf("callRPC %v: %v", mVBGod, err)
	}
}

func (g *gVBGrpc) addPeer(peer *gVBGrpc) {
	g.gVBG.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gVBG.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gVBG.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gVBG.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gVBG.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *gVBGrpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func (g *gVBGrpc) waitSynced() {
	// Check if it's synced now
	var result interface{}
	g.callRPC(&result, "VBG_syncing")
	syncing, ok := result.(bool)
	if ok && !syncing {
		g.gVBG.Logf("%v already synced", g.name)
		return
	}

	// Actually wait, subscribe to the event
	ch := make(chan interface{})
	sub, err := g.rpc.Subscribe(context.Background(), "VBG", ch, "syncing")
	if err != nil {
		g.gVBG.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.gVBG.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.gVBG.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.gVBG.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.gVBG.Fatalf("%v timeout syncing", g.name)
		break
	}
}
...
func startGVBGWithIpc(t *testing.T, name string, args ...string) *gVBGrpc {
	ipcName := fmt.Sprintf("gVBG-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	...
	var err error
	ipcpath := ipcEndpoint(ipcName, g.gVBG.Datadir)
...
func initGVBG(t *testing.T) string {
	args := []string{"--nousb", "--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing gVBG: %v ", args)
	g := runGVBG(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *gVBGrpc {
	datadir := initGVBG(t)
	t.Logf("Importing keys to gVBG")
	runGVBG(t, "--nousb", "--datadir", datadir, "--password", "./testdata/password.txt", "account", "import", "./testdata/key.prv", "--lightkdf").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGVBGWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *gVBGrpc {
	datadir := initGVBG(t)
	return startGVBGWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
