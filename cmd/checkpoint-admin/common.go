...
func newClient(ctx *cli.Context) *VBGclient.Client {
	client, err := VBGclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
		utils.Fatalf("Failed to connect to vbgloble node: %v", err)
	}
	return client
}
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, VBGclient.NewClient(client))
	if err != nil {
