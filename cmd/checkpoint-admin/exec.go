...
import (
	...
	"github.com/vbgloble/go-VGB/contracts/checkpointoracle"
...
func sign(ctx *cli.Context) error {
	var (
		offline bool // The indicator whVBGer we sign checkpoint by offline.
		chash   common.Hash
		cindex  uint64
	...
	if !ctx.GlobalIsSet(nodeURLFlag.Name) {
		...
		head, err := VBGclient.NewClient(node).HeaderByNumber(reqCtx, nil)
		if err != nil {
			return err
		}
	...
	// isAdmin checks whVBGer the specified signer is admin.
...
func publish(ctx *cli.Context) error {
	...
	head, err := VBGclient.NewClient(client).HeaderByNumber(reqCtx, nil)
	if err != nil {
		return err
	}
	num := head.Number.Uint64()
	recent, err := VBGclient.NewClient(client).HeaderByNumber(reqCtx, big.NewInt(int64(num-128)))
	if err != nil {
		return err
	}
