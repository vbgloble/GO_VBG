...
func TestMessageSignVerify(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "VBGkey-test")
	if err != nil {
	...
	// Create the key.
	generate := runVBGkey(t, "generate", "--lightkdf", keyfile)
	generate.Expect(`
...
`)
	...
	// Sign a message.
	sign := runVBGkey(t, "signmessage", keyfile, message)
	sign.Expect(`
!! Unsupported terminal, password will be echoed.
Password: {{.InputLine "foobar"}}
`)
	...
	// Verify the message.
	verify := runVBGkey(t, "verifymessage", address, signature, message)
