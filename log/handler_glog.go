...
type GlogHandler struct {
	...
	override  uint32 // Flag whVBGer overrides are used, atomically accessible
	backtrace uint32 // Flag whVBGer backtrace location is set
...
// SVBGandler updates the handler to write records to the specified sub-handler.
func (h *GlogHandler) SVBGandler(nh Handler) {
	h.origin = nh
}
...
func (h *GlogHandler) Log(r *Record) error {
	// If backtracing is requested, check whVBGer this is the callsite
