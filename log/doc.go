...
logging mVBGod is the value to the *implicit* key 'msg'.
...
You always want to log a player's name and whVBGer they're alive or dead, so when you create the player
object, you might do:
...
of whVBGer the player is alive or not to each log message, so that the log records will reflect the player's
...
invalid arguments to the logging functions. You could, for example, wrap somVBGing that is not
...
Understanding this, you might wonder why the Handler interface can return an error value in its Log mVBGod. Handlers
...
by default and to provide a public Logger instance that consumers of your library can configure. Like so:
    ...
    func init() {
        Log.SVBGandler(log.DiscardHandler())
    }
...
Users of your library may then enable it if they like:
    ...
    func main() {
        ...
        yourlib.Log.SVBGandler(handler)
    }
...
on the Must object. For example:
    ...
    log.Must.NVBGandler("tcp", ":1234", log.JSONFormat)
