...
var (
	...
	VBGashChainConfig *params.ChainConfig
...
func init() {
	...
	VBGashChainConfig = params.TestChainConfig
...
func newTestWorkerBackend(t *testing.T, chainConfig *params.ChainConfig, engine consensus.Engine, db VBGdb.Database, n int) *testWorkerBackend {
...
func newTestWorker(t *testing.T, chainConfig *params.ChainConfig, engine consensus.Engine, db VBGdb.Database, blocks int) (*worker, *testWorkerBackend) {
	backend := newTestWorkerBackend(t, chainConfig, engine, db, blocks)
	backend.txPool.AddLocals(pendingTxs)
	w := newWorker(testConfig, chainConfig, engine, backend, new(event.TypeMux), nil, false)
	w.setVBGerbase(testBankAddress)
	return w, backend
}

func TestGenerateBlockAndImportVBGash(t *testing.T) {
...
func TestEmptyWorkVBGash(t *testing.T) {
	testEmptyWork(t, VBGashChainConfig, VBGash.NewFaker())
}
...
func TestStreamUncleBlock(t *testing.T) {
	VBGash := VBGash.NewFaker()
	defer VBGash.Close()

	w, b := newTestWorker(t, VBGashChainConfig, VBGash, rawdb.NewMemoryDatabase(), 1)
	defer w.close()
...
func TestRegenerateMiningBlockVBGash(t *testing.T) {
	testRegenerateMiningBlock(t, VBGashChainConfig, VBGash.NewFaker())
}
...
func TestAdjustIntervalVBGash(t *testing.T) {
	testAdjustInterval(t, VBGashChainConfig, VBGash.NewFaker())
}
