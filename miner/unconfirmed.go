...
// chainRetriever is used by the unconfirmed block set to verify whVBGer a previously
// mined block is part of the canonical chain or not.
type chainRetriever interface {
	// GVBGeaderByNumber retrieves the canonical header associated with a block number.
	GVBGeaderByNumber(number uint64) *types.Header
...
func (set *unconfirmedBlocks) Shift(height uint64) {
	...
	for set.blocks != nil {
		...
		header := set.chain.GVBGeaderByNumber(next.index)
		switch {
		case header == nil:
		...
		default:
			// Block is not canonical, check whVBGer we have an uncle or a lost block
			included := false
