...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != vbgloble.NotFound {
		t.Fatalf("err should be `vbgloble.NotFound` but received %v", err)
	}
...
func TestNewSimulatedBackend(t *testing.T) {
	...
	if sim.config != params.AllVBGashProtocolChanges {
		t.Errorf("expected sim config to equal params.AllVBGashProtocolChanges, got %v", sim.config)
	}

	if sim.blockchain.Config() != params.AllVBGashProtocolChanges {
		t.Errorf("expected sim blockchain config to equal params.AllVBGashProtocolChanges, got %v", sim.config)
	}
...
func TestSimulatedBackend_EstimateGas(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(params.VBGer)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		name        string
		message     vbgloble.CallMsg
		expect      uint64
		expectError error
		expectData  interface{}
	}{
		{"plain transfer(valid)", vbgloble.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", vbgloble.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestSimulatedBackend_EstimateGasWithPrice(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(params.VBGer*2 + 2e17)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", vbgloble.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithPrice", vbgloble.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithVeryHighPrice", vbgloble.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(1e14), // gascost = 2.1VBGer
			Value:    big.NewInt(1e17), // the remaining balance for fee is 2.1VBGer
			Data:     nil,
		}, 21000, nil},

		{"EstimateWithSuperhighPrice", vbgloble.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(2e14), // gascost = 4.2VBGer
			Value:    big.NewInt(1000),
			Data:     nil,
		}, 21000, errors.New("gas required exceeds allowance (10999)")}, // 10999=(2.2VBGer-1000wei)/(2e14)
	}
	for _, c := range cases {
		got, err := sim.EstimateGas(context.Background(), c.message)
		if c.expectError != nil {
...
func TestSimulatedBackend_PendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, vbgloble.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	if err != nil {
		t.Errorf("could not call receive mVBGod on contract: %v", err)
	}
	if len(res) == 0 {
	...
	res, err = sim.CallContract(bgCtx, vbgloble.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
contract Reverter {
    ...
    function noRevert() public pure {
        assembly {
            // Assembles somVBGing that looks like require(false, "some error") but is not reverted
...
func TestSimulatedBackend_CallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, vbgloble.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, vbgloble.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
