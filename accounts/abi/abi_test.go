...
import (
	...
	"github.com/vbgloble/go-VGB/common/math"
...
var mVBGods = map[string]MVBGod{
	"":                    NewMVBGod("", "", Function, "", false, false, nil, nil),
	"balance":             NewMVBGod("balance", "balance", Function, "view", false, false, nil, nil),
	"send":                NewMVBGod("send", "send", Function, "", false, false, []Argument{{"amount", Uint256, false}}, nil),
	"test":                NewMVBGod("test", "test", Function, "", false, false, []Argument{{"number", Uint32, false}}, nil),
	"string":              NewMVBGod("string", "string", Function, "", false, false, []Argument{{"inputs", String, false}}, nil),
	"bool":                NewMVBGod("bool", "bool", Function, "", false, false, []Argument{{"inputs", Bool, false}}, nil),
	"address":             NewMVBGod("address", "address", Function, "", false, false, []Argument{{"inputs", Address, false}}, nil),
	"uint64[]":            NewMVBGod("uint64[]", "uint64[]", Function, "", false, false, []Argument{{"inputs", Uint64Arr, false}}, nil),
	"uint64[2]":           NewMVBGod("uint64[2]", "uint64[2]", Function, "", false, false, []Argument{{"inputs", Uint64Arr2, false}}, nil),
	"int8":                NewMVBGod("int8", "int8", Function, "", false, false, []Argument{{"inputs", Int8, false}}, nil),
	"foo":                 NewMVBGod("foo", "foo", Function, "", false, false, []Argument{{"inputs", Uint32, false}}, nil),
	"bar":                 NewMVBGod("bar", "bar", Function, "", false, false, []Argument{{"inputs", Uint32, false}, {"string", Uint16, false}}, nil),
	"slice":               NewMVBGod("slice", "slice", Function, "", false, false, []Argument{{"inputs", Uint32Arr2, false}}, nil),
	"slice256":            NewMVBGod("slice256", "slice256", Function, "", false, false, []Argument{{"inputs", Uint256Arr2, false}}, nil),
	"sliceAddress":        NewMVBGod("sliceAddress", "sliceAddress", Function, "", false, false, []Argument{{"inputs", AddressArr, false}}, nil),
	"sliceMultiAddress":   NewMVBGod("sliceMultiAddress", "sliceMultiAddress", Function, "", false, false, []Argument{{"a", AddressArr, false}, {"b", AddressArr, false}}, nil),
	"nestedArray":         NewMVBGod("nestedArray", "nestedArray", Function, "", false, false, []Argument{{"a", Uint256ArrNested, false}, {"b", AddressArr, false}}, nil),
	"nestedArray2":        NewMVBGod("nestedArray2", "nestedArray2", Function, "", false, false, []Argument{{"a", Uint8ArrNested, false}}, nil),
	"nestedSlice":         NewMVBGod("nestedSlice", "nestedSlice", Function, "", false, false, []Argument{{"a", Uint8SliceNested, false}}, nil),
	"receive":             NewMVBGod("receive", "receive", Function, "payable", false, true, []Argument{{"memo", Bytes, false}}, []Argument{}),
	"fixedArrStr":         NewMVBGod("fixedArrStr", "fixedArrStr", Function, "view", false, false, []Argument{{"str", String, false}, {"fixedArr", Uint256Arr2, false}}, nil),
	"fixedArrBytes":       NewMVBGod("fixedArrBytes", "fixedArrBytes", Function, "view", false, false, []Argument{{"bytes", Bytes, false}, {"fixedArr", Uint256Arr2, false}}, nil),
	"mixedArrStr":         NewMVBGod("mixedArrStr", "mixedArrStr", Function, "view", false, false, []Argument{{"str", String, false}, {"fixedArr", Uint256Arr2, false}, {"dynArr", Uint256Arr, false}}, nil),
	"doubleFixedArrStr":   NewMVBGod("doubleFixedArrStr", "doubleFixedArrStr", Function, "view", false, false, []Argument{{"str", String, false}, {"fixedArr1", Uint256Arr2, false}, {"fixedArr2", Uint256Arr3, false}}, nil),
	"multipleMixedArrStr": NewMVBGod("multipleMixedArrStr", "multipleMixedArrStr", Function, "view", false, false, []Argument{{"str", String, false}, {"fixedArr1", Uint256Arr2, false}, {"dynArr", Uint256Arr, false}, {"fixedArr2", Uint256Arr3, false}}, nil),
	"overloadedNames":     NewMVBGod("overloadedNames", "overloadedNames", Function, "view", false, false, []Argument{{"f", TupleF, false}}, nil),
}
...
func TestReader(t *testing.T) {
	...
	for name, expM := range exp.MVBGods {
		gotM, exist := abi.MVBGods[name]
		if !exist {
			t.Errorf("Missing expected mVBGod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi mVBGod: \n%v\ndoes not match expected mVBGod\n%v", gotM, expM)
		}
	}

	for name, gotM := range abi.MVBGods {
		expM, exist := exp.MVBGods[name]
		if !exist {
			t.Errorf("Found extra mVBGod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi mVBGod: \n%v\ndoes not match expected mVBGod\n%v", gotM, expM)
		}
...
func TestConstructor(t *testing.T) {
	...
	mVBGod := NewMVBGod("", "", Constructor, "nonpayable", false, false, []Argument{{"a", Uint256, false}, {"b", Uint256, false}}, nil)
	// Test from JSON
	abi, err := JSON(strings.NewReader(json))
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(abi.Constructor, mVBGod) {
...
func TestMVBGodSignature(t *testing.T) {
	m := NewMVBGod("foo", "foo", Function, "", false, false, []Argument{{"bar", String, false}, {"baz", String, false}}, nil)
	...
	m = NewMVBGod("foo", "foo", Function, "", false, false, []Argument{{"bar", Uint256, false}}, nil)
	exp = "foo(uint256)"
	if m.Sig != exp {
		t.Error("signature mismatch", exp, "!=", m.Sig)
	}

	// MVBGod with tuple arguments
	...
	m = NewMVBGod("foo", "foo", Function, "", false, false, []Argument{{"s", s, false}, {"bar", String, false}}, nil)
...
func TestOverloadedMVBGodSignature(t *testing.T) {
	json := `[{"constant":true,"inputs":[{"name":"i","type":"uint256"},{"name":"j","type":"uint256"}],"name":"foo","outputs":[],"payable":false,"stateMutability":"pure","type":"function"},{"constant":true,"inputs":[{"name":"i","type":"uint256"}],"name":"foo","outputs":[],"payable":false,"stateMutability":"pure","type":"function"},{"anonymous":false,"inputs":[{"indexed":false,"name":"i","type":"uint256"}],"name":"bar","type":"event"},{"anonymous":false,"inputs":[{"indexed":false,"name":"i","type":"uint256"},{"indexed":false,"name":"j","type":"uint256"}],"name":"bar","type":"event"}]`
	abi, err := JSON(strings.NewReader(json))
	if err != nil {
		t.Fatal(err)
	}
	check := func(name string, expect string, mVBGod bool) {
		if mVBGod {
			if abi.MVBGods[name].Sig != expect {
				t.Fatalf("The signature of overloaded mVBGod mismatch, want %s, have %s", expect, abi.MVBGods[name].Sig)
			}
		} else {
...
func TestDefaultFunctionParsing(t *testing.T) {
	...
	if _, ok := abi.MVBGods["balance"]; !ok {
...
func TestUnpackMVBGodIntoMap(t *testing.T) {
	...
	// Tests a mVBGod with no outputs
	...
	// Tests a mVBGod with only outputs
	...
	// Tests a mVBGod with outputs and inputs
...
func TestUnpackIntoMapNamingConflict(t *testing.T) {
	// Two mVBGods have the same name
	...
	getMap := map[string]interface{}{}
	if err = abi.UnpackIntoMap(getMap, "get", data); err == nil {
		t.Error("naming conflict between two mVBGods; error expected")
	}
	...
	// MVBGod and event have the same name
	...
	if err = abi.UnpackIntoMap(receivedMap, "received", data); err == nil {
		t.Error("naming conflict between an event and a mVBGod; error expected")
	}
...
func TestABI_MVBGodById(t *testing.T) {
	abi, err := JSON(strings.NewReader(jsondata))
	if err != nil {
		t.Fatal(err)
	}
	for name, m := range abi.MVBGods {
		a := fmt.Sprintf("%v", m)
		m2, err := abi.MVBGodById(m.ID)
		if err != nil {
			t.Fatalf("Failed to look up ABI mVBGod: %v", err)
		}
		b := fmt.Sprintf("%v", m2)
		if a != b {
			t.Errorf("MVBGod %v (id %x) not 'findable' by id in ABI", name, m.ID)
		}
	}
	// test unsuccessful lookups
	if _, err = abi.MVBGodById(crypto.Keccak256()); err == nil {
		t.Error("Expected error: no mVBGod with this id")
	}
	// Also test empty
	if _, err := abi.MVBGodById([]byte{0x00}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.MVBGodById([]byte{}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.MVBGodById(nil); err == nil {
...
func TestABI_EventById(t *testing.T) {
	...
	for testnum, test := range tests {
		...
		if err != nil {
			t.Fatalf("Failed to look up ABI mVBGod: %v, test #%d", err, testnum)
		}
		if event == nil {
...
// TestDoubleDuplicateMVBGodNames checks that if transfer0 already exists, there won't be a name
// conflict and that the second transfer mVBGod will be renamed transfer1.
func TestDoubleDuplicateMVBGodNames(t *testing.T) {
	abiJSON := `[{"constant":false,"inputs":[{"name":"to","type":"address"},{"name":"value","type":"uint256"}],"name":"transfer","outputs":[{"name":"ok","type":"bool"}],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":false,"inputs":[{"name":"to","type":"address"},{"name":"value","type":"uint256"},{"name":"data","type":"bytes"}],"name":"transfer0","outputs":[{"name":"ok","type":"bool"}],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":false,"inputs":[{"name":"to","type":"address"},{"name":"value","type":"uint256"},{"name":"data","type":"bytes"},{"name":"customFallback","type":"string"}],"name":"transfer","outputs":[{"name":"ok","type":"bool"}],"payable":false,"stateMutability":"nonpayable","type":"function"}]`
	contractAbi, err := JSON(strings.NewReader(abiJSON))
	if err != nil {
		t.Fatal(err)
	}
	if _, ok := contractAbi.MVBGods["transfer"]; !ok {
		t.Fatalf("Could not find original mVBGod")
	}
	if _, ok := contractAbi.MVBGods["transfer0"]; !ok {
		t.Fatalf("Could not find duplicate mVBGod")
	}
	if _, ok := contractAbi.MVBGods["transfer1"]; !ok {
		t.Fatalf("Could not find duplicate mVBGod")
	}
	if _, ok := contractAbi.MVBGods["transfer2"]; ok {
		t.Fatalf("Should not have found extra mVBGod")
	}
}
