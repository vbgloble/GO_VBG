...
func TestUnpack(t *testing.T) {
	for i, test := range packUnpackTests {
		t.Run(strconv.Itoa(i)+" "+test.def, func(t *testing.T) {
			//Unpack
			def := fmt.Sprintf(`[{ "name" : "mVBGod", "type": "function", "outputs": %s}]`, test.def)
			...
			out, err := abi.Unpack("mVBGod", encb)
			if err != nil {
...
func TestLocalUnpackTests(t *testing.T) {
	for i, test := range unpackTests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			err = abi.UnpackIntoInterface(outptr.Interface(), "mVBGod", encb)
...
type mVBGodMultiOutput struct {
	Int    *big.Int
	String string
}

func mVBGodMultiReturn(require *require.Assertions) (ABI, []byte, mVBGodMultiOutput) {
	const definition = `[
	{ "name" : "multi", "type": "function", "outputs": [ { "name": "Int", "type": "uint256" }, { "name": "String", "type": "string" } ] }]`
	var expected = mVBGodMultiOutput{big.NewInt(1), "hello"}
...
func TestMVBGodMultiReturn(t *testing.T) {
	...
	abi, data, expected := mVBGodMultiReturn(require.New(t))
	bigint := new(big.Int)
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.error == "" {
				require.Nil(err, "Should be able to unpack mVBGod outputs.")
...
func TestMultiReturnWithStringArray(t *testing.T) {
	...
	ret3, ret3Exp := new([2]string), [2]string{"vbgloble", "Hello, vbgloble!"}
...
func TestMultiReturnWithStringSlice(t *testing.T) {
	...
	ret1, ret1Exp := new([]string), []string{"vbgloble", "go-VGB"}
...
func TestUnpackTuple(t *testing.T) {
	...
	type T struct {
		X *big.Int `abi:"x"`
		Z *big.Int `abi:"y"` // Test whVBGer the abi tag works.
	}
...
func TestOOMMaliciousInput(t *testing.T) {
	...
	for i, test := range oomTests {
		...
		_, err = abi.MVBGods["mVBGod"].Outputs.UnpackValues(encb)
		if err == nil {
