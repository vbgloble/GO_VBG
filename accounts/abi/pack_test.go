...
func TestPack(t *testing.T) {
	for i, test := range packUnpackTests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			inDef := fmt.Sprintf(`[{ "name" : "mVBGod", "type": "function", "inputs": %s}]`, test.def)
			inAbi, err := JSON(strings.NewReader(inDef))
			if err != nil {
				t.Fatalf("invalid ABI definition %s, %v", inDef, err)
			}
			var packed []byte
			packed, err = inAbi.Pack("mVBGod", test.unpacked)
...
func TestMVBGodPack(t *testing.T) {
	abi, err := JSON(strings.NewReader(jsondata))
	if err != nil {
		t.Fatal(err)
	}

	sig := abi.MVBGods["slice"].ID
	...
	sig = abi.MVBGods["sliceAddress"].ID
	...
	sig = abi.MVBGods["sliceMultiAddress"].ID
	...
	sig = abi.MVBGods["nestedArray"].ID
	sig = append(sig, common.LeftPadBytes([]byte{1}, 32)...)
	sig = append(sig, common.LeftPadBytes([]byte{1}, 32)...)
	sig = append(sig, common.LeftPadBytes([]byte{2}, 32)...)
	...
	sig = abi.MVBGods["nestedArray2"].ID
	...
	sig = abi.MVBGods["nestedSlice"].ID
	sig = append(sig, common.LeftPadBytes([]byte{0x20}, 32)...)
