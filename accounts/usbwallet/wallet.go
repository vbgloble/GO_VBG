...
const selfDerivVBGrottling = time.Second
...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the vbgloble
...
type wallet struct {
	...
	deriveChain     vbgloble.ChainStateReader // Blockchain state reader to discover used account with
...
// whVBGer they are still present or if they malfunctioned.
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived vbgloble account
		...
		if err == nil {
			...
			case <-time.After(selfDerivVBGrottling):
...
// Contains implements accounts.Wallet, returning whVBGer a particular account is
...
// this mVBGod supports providing multiple bases to discover old user accounts
// too. Only the last base will be used to derive the next empty account.
//
// You can disable automatic account discovery by calling SelfDerive with a nil
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain vbgloble.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
// supported for hardware wallets, so this mVBGod will always return an error.
...
// data is not supported for Ledger wallets, so this mVBGod will always return
// an error.
