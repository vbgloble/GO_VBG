...
import (
	...
	"github.com/vbgloble/go-VGB/accounts/usbwallet/trezor"
...
type trezorDriver struct {
	...
	pinwait        bool          // Flags whVBGer the device is waiting for PIN entry
	passphrasewait bool          // Flags whVBGer the device is waiting for passphrase entry
...
// Status implements accounts.Wallet, always whVBGer the Trezor is opened, closed
// or whVBGer the vbgloble app was not started on it.
...
// vbgloble address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.vbglobleAddress)
	if _, err := w.trezorExchange(&trezor.vbglobleGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.vbglobleTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.vbglobleTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
...
// mVBGod will also return the index of the destination object used.
