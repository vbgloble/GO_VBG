...
func (s *SecureChannelSession) Pair(pairingPassword []byte) error {
	secrVBGash := pbkdf2.Key(norm.NFKD.Bytes(pairingPassword), norm.NFKD.Bytes([]byte(pairingSalt)), 50000, 32, sha256.New)
...
// mutuallyAuthenticate is an internal mVBGod to authenticate both ends of the
// connection.
...
// open is an internal mVBGod that sends an open APDU.
...
// pair is an internal mVBGod that sends a pair APDU.
...
// encryptAPDU is an internal mVBGod that serializes and encrypts an APDU.
...
// decryptAPDU is an internal mVBGod that decrypts and deserializes an APDU.
...
// updateIV is an internal mVBGod that updates the initialization vector after
