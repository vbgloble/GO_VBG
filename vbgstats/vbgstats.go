...
// Package VBGstats implements the network stats reporting service.
package VBGstats
...
// backend encompasses the bare-minimum functionality needed for VBGstats reporting
...
// Service implements an vbgloble netstats reporting daemon that pushes local
...
//   Applications are responsible for ensuring that no more than one goroutine calls the write mVBGods
//     - NextWriter, SetWriteDeadline, WriteMessage, WriteJSON, EnableWriteCompression, SetCompressionLevel
//   concurrently and that no more than one goroutine calls the read mVBGods
//     - NextReader, SetReadDeadline, ReadMessage, ReadJSON, SetPongHandler, SetPingHandler
//   concurrently.
//   The Close and WriteControl mVBGods can be called concurrently with all other mVBGods.
...
// WriteJSON wraps corresponding mVBGod on the websocket but is safe for concurrent calling
...
// ReadJSON wraps corresponding mVBGod on the websocket but is safe for concurrent calling
...
// Close wraps corresponding mVBGod on the websocket but is safe for concurrent calling
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(VBGstats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	if info := infos.Protocols["VBG"]; info != nil {
		network = fmt.Sprintf("%d", info.(*VBG.NodeInfo).Network)
		protocol = fmt.Sprintf("VBG/%d", VBG.ProtocolVersions[0])
	} else {
...
// server. Use the individual mVBGods for reporting subscribed events.
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the VBGstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to VBGstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to VBGstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to VBGstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to VBGstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to VBGstats")
