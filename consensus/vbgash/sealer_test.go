...
import (
	...
	"github.com/vbgloble/go-VGB/internal/testlog"
	"github.com/vbgloble/go-VGB/log"
)

// Tests whVBGer remote HTTP servers are correctly notified of new work.
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom VBGash engine.
	VBGash := NewTester([]string{server.URL}, false)
	defer VBGash.Close()
	...
	VBGash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := VBGash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom VBGash engine.
	VBGash := NewTester([]string{server.URL}, false)
	VBGash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer VBGash.Close()
...
// Tests whVBGer stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	VBGash := NewTester(nil, true)
	defer VBGash.Close()
	api := &API{VBGash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			VBGash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, VBGash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch VBGash result timeout", id+1)
		}
	}
}
