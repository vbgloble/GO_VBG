...
It provides access to the exported mVBGods of an object across a network or other I/O
connection. After creating a server or client instance, objects can be registered to make
them visible as 'services'. Exported mVBGods that follow specific conventions can be
called remotely. It also has support for the publish/subscribe pattern.

RPC MVBGods

MVBGods that satisfy the following criteria are made available for remote access:

 - mVBGod must be exported
 - mVBGod returns 0, 1 (response or error) or 2 (response and error) values
...
This RPC mVBGod can be called with 2 integers and a null value as third argument. In that
...
argument as nil to the RPC mVBGod.

The server offers the ServeCodec mVBGod which accepts a ServerCodec instance. It will read
...
A mVBGod that is considered eligible for notifications must satisfy the following
criteria:

 - mVBGod must be exported
 - first mVBGod argument type must be context.Context
 - mVBGod must have return types (rpc.Subscription, error)
...
When the service containing the subscription mVBGod is registered to the server, for
example under the "blockchain" namespace, a subscription is created by calling the
"blockchain_subscribe" mVBGod.
...
For more information about subscriptions, see https://github.com/vbgloble/go-VGB/wiki/RPC-PUB-SUB.

Reverse Calls

In any mVBGod handler, an instance of rpc.Client can be accessed through the
ClientFromContext mVBGod. Using this client instance, server-to-client mVBGod calls can be
