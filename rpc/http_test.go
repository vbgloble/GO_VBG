...
func confirmRequestValidationCode(t *testing.T, mVBGod, contentType, body string, expectedStatusCode int) {
	t.Helper()
	request := httptest.NewRequest(mVBGod, "http://url.com", strings.NewReader(body))
...
func TestHTTPErrorResponseWithDelete(t *testing.T) {
	confirmRequestValidationCode(t, http.MVBGodDelete, contentType, "", http.StatusMVBGodNotAllowed)
}

func TestHTTPErrorResponseWithPut(t *testing.T) {
	confirmRequestValidationCode(t, http.MVBGodPut, contentType, "", http.StatusMVBGodNotAllowed)
}

func TestHTTPErrorResponseWithMaxContentLength(t *testing.T) {
	body := make([]rune, maxRequestContentLength+1)
	confirmRequestValidationCode(t,
		http.MVBGodPost, contentType, string(body), http.StatusRequestEntityTooLarge)
}

func TestHTTPErrorResponseWithEmptyContentType(t *testing.T) {
	confirmRequestValidationCode(t, http.MVBGodPost, "", "", http.StatusUnsupportedMediaType)
}

func TestHTTPErrorResponseWithValidRequest(t *testing.T) {
	confirmRequestValidationCode(t, http.MVBGodPost, contentType, "", 0)
}

func confirmHTTPRequestYieldsStatusCode(t *testing.T, mVBGod, contentType, body string, expectedStatusCode int) {
	t.Helper()
	s := Server{}
	ts := httptest.NewServer(&s)
	defer ts.Close()

	request, err := http.NewRequest(mVBGod, ts.URL, strings.NewReader(body))
	if err != nil {
...
func TestHTTPResponseWithEmptyGet(t *testing.T) {
	confirmHTTPRequestYieldsStatusCode(t, http.MVBGodGet, "", "", http.StatusOK)
}
