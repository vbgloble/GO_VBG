...
func TestClientBatchRequest(t *testing.T) {
	...
	wantResult := []BatchElem{
		...
		},
		{
			...
			Args:   []interface{}{1, 2, 3},
			Result: new(int),
			Error:  &jsonError{Code: -32601, Message: "the mVBGod no_such_mVBGod does not exist/is not available"},
		},
	}
	...
	check := func(shouldPanic bool, arg interface{}) {
		defer func() {
			...
			if shouldPanic && err == nil {
				t.Errorf("VBGSubscribe should've panicked for %#v", arg)
			}
			if !shouldPanic && err != nil {
				t.Errorf("VBGSubscribe shouldn't have panicked for %#v", arg)
		...
		client.VBGSubscribe(context.Background(), arg, "foo_bar")
	}
	check(true, nil)
	check(true, 1)
...
// This test reproduces https://github.com/vbgloble/go-VGB/issues/17837 where the
...
func TestClientSVBGeader(t *testing.T) {
	var gotHeader bool
	srv := newTestServer()
	...
	client.SVBGeader("content-type", "application/x-garbage")
...
func TestClientReconnect(t *testing.T) {
	startServer := func(addr string) (*Server, net.Listener) {
		...
		go http.Serve(l, srv.WebsockVBGandler([]string{"*"}))
		return srv, l
	}
	...
	// We spawn multiple calls here to check whVBGer this hangs somehow.
...
func httpTestClient(srv *Server, transport string, fl *flakeyListener) (*Client, *httptest.Server) {
	...
	switch transport {
	case "ws":
		hs = httptest.NewUnstartedServer(srv.WebsockVBGandler([]string{"*"}))
	case "http":
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-VGB-test-ipc-%d-%d", os.Getpid(), rand.Int63())
