...
func TestWebsocketOriginCheck(t *testing.T) {
	...
	var (
		srv     = newTestServer()
		httpsrv = httptest.NewServer(srv.WebsockVBGandler([]string{"http://example.com"}))
...
// This test checks whVBGer calls exceeding the request size limit are rejected.
func TestWebsocketLargeCall(t *testing.T) {
	t.Parallel()

	var (
		srv     = newTestServer()
		httpsrv = httptest.NewServer(srv.WebsockVBGandler([]string{"*"}))
		wsURL   = "ws:" + strings.TrimPrefix(httpsrv.URL, "http:")
	)
	defer srv.Stop()
	defer httpsrv.Close()
...
func TestClientWebsocketPing(t *testing.T) {
	...
	sub, err := client.VBGSubscribe(ctx, resultChan, "foo")
	if err != nil {
		t.Fatalf("client subscribe error: %v", err)
	}

	// Wait for the context's deadline to be reached before proceeding.
	// This is important for reproducing https://github.com/vbgloble/go-VGB/issues/19798
	<-ctx.Done()
	close(sendPing)
...
func wsPingTestHandler(t *testing.T, conn *websocket.Conn, shutdown, sendPing <-chan struct{}) {
	// Canned responses for the VBG_subscribe call in TestClientWebsocketPing.
	const (
		subResp   = `{"jsonrpc":"2.0","id":1,"result":"0x00"}`
		subNotify = `{"jsonrpc":"2.0","mVBGod":"VBG_subscription","params":{"subscription":"0x00","result":1}}`
	)
