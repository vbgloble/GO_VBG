...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte vbgloble address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an vbgloble account at a particular block.
    type Account {
    ...
    # Transaction is an vbgloble transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an vbgloble account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an vbgloble account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an vbgloble block by number or by hash. If neither is
