...
func TestGraphQLHTTPOnSamePort_GQLRequest_Successful(t *testing.T) {
	...
	gqlReq, err := http.NewRequest(http.MVBGodGet, fmt.Sprintf("http://%s/graphql", "127.0.0.1:9393"), body)
	if err != nil {
...
func TestGraphQLHTTPOnSamePort_GQLRequest_Unsuccessful(t *testing.T) {
	...
	// create http request
	body := strings.NewReader("{\"query\": \"{block{number}}\",\"variables\": null}")
	gqlReq, err := http.NewRequest(http.MVBGodPost, fmt.Sprintf("http://%s/graphql", "127.0.0.1:9393"), body)
	if err != nil {
		t.Error("could not issue new http request ", err)
	}
	gqlReq.Header.Set("Content-Type", "application/json")
...
func createGQLService(t *testing.T, stack *node.Node, endpoint string) {
	...
	VBGConf := &VBG.Config{
		...
		Miner: miner.Config{
			VBGerbase: common.HexToAddress("0xaabb"),
		},
	...
	if err != nil {
		t.Fatalf("could not create VBG backend: %v", err)
	}

	// create gql service
	err = New(stack, VBGBackend.APIBackend, []string{}, []string{})
	if err != nil {
