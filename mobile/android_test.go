...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", GVBG.LightScryptN, GVBG.LightScryptP);
		...
		try {
			...
			// data from this mVBGod invocation is a JSON encoded, encrypted key-file.
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.vbgloble", new NodeConfig());
			node.start();
			...
			vbglobleClient ec = node.getvbglobleClient();
	...
	// signatures too. Regression test for go-VGB issue #14599.
...
//
// This mVBGod has been adapted from golang.org/x/mobile/bind/java/seq_test.go/runTest
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogVBGer
	...
	workspace, err := ioutil.TempDir("", "gVBG-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/vbgloble/gVBGtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for GVBG and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.vbgloble", "github.com/vbgloble/go-VGB/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gVBG.aar"), "gVBG.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "vbgloble", "gVBGtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
