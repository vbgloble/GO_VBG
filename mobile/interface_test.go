...
func TestInterfaceGetSet(t *testing.T) {
	...
	callFn := func(receiver interface{}, mVBGod string, arg interface{}) interface{} {
		rval := reflect.ValueOf(receiver)
		rval.MVBGodByName(fmt.Sprintf("Set%s", mVBGod)).Call([]reflect.Value{reflect.ValueOf(arg)})
		res := rval.MVBGodByName(fmt.Sprintf("Get%s", mVBGod)).Call(nil)
		if len(res) > 0 {
	...
	for index, c := range tests {
		...
		result := callFn(iface, c.mVBGod, c.input)
		if !reflect.DeepEqual(result, c.expect) {
			t.Errorf("Interface get/set mismatch, want %v, got %v", c.expect, result)
		}
		// Check whVBGer the underlying value in args is still zero
		iface, _ = args.Get(index)
		if iface.object != nil {
