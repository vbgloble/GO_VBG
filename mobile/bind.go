...
// mVBGod to sign the transaction before submission.
...
// NewKeyedTransactOpts is a utility mVBGod to easily create a transaction signer
...
func DeployContract(opts *TransactOpts, abiJSON string, bytecode []byte, client *vbglobleClient, args *Interfaces) (contract *BoundContract, _ error) {
...
func BindContract(address *Address, abiJSON string, client *vbglobleClient) (contract *BoundContract, _ error) {
	parsed, err := abi.JSON(strings.NewReader(abiJSON))
	if err != nil {
		return nil, err
	}
...
func (c *BoundContract) Call(opts *CallOpts, out *Interfaces, mVBGod string, args *Interfaces) error {
	results := make([]interface{}, len(out.objects))
	copy(results, out.objects)
	if err := c.contract.Call(&opts.opts, &results, mVBGod, args.objects...); err != nil {
		return err
	}
...
func (c *BoundContract) Transact(opts *TransactOpts, mVBGod string, args *Interfaces) (tx *Transaction, _ error) {
	rawTx, err := c.contract.Transact(&opts.opts, mVBGod, args.objects...)
	if err != nil {
		return nil, err
	}
...
// RawTransact invokes the (paid) contract mVBGod with raw calldata as input values.
