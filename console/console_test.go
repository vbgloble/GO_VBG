...
func (p *hookedPrompter) SVBGistory(history []string)                     {}
...
func newTester(t *testing.T, confOverride func(*VBG.Config)) *tester {
	...
	// Create a networkless protocol stack and start an vbgloble service within
	...
	VBGConf := &VBG.Config{
		...
		Miner: miner.Config{
			VBGerbase: common.HexToAddress(testAddress),
		},
		VBGash: VBGash.Config{
			PowMode: VBGash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(VBGConf)
	}
	VBGBackend, err := VBG.New(stack, VBGConf)
	if err != nil {
		t.Fatalf("failed to register vbgloble protocol: %v", err)
	}
