...
import (
	...
	"github.com/vbgloble/go-VGB/accounts/usbwallet"
	"github.com/vbgloble/go-VGB/common/hexutil"
	"github.com/vbgloble/go-VGB/console/prompt"
	"github.com/vbgloble/go-VGB/internal/jsre"
	"github.com/vbgloble/go-VGB/rpc"
)

// bridge is a collection of JavaScript utility mVBGods to bride the .js runtime
// environment and the Go RPC connection backing the remote mVBGod calls.
type bridge struct {
	client   *rpc.Client         // RPC client to execute vbgloble requests through
...
func getJVBG(vm *goja.Runtime) *goja.Object {
	jVBG := vm.Get("jVBG")
	if jVBG == nil {
		panic(vm.ToValue("jVBG object does not exist"))
	}
	return jVBG.ToObject(vm)
}

// NewAccount is a wrapper around the personal.newAccount RPC mVBGod that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC mVBGod (saved in jVBG.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call jsre.Call) (goja.Value, error) {
	...
	newAccount, callable := goja.AssertFunction(getJVBG(call.VM).Get("newAccount"))
	if !callable {
		return nil, fmt.Errorf("jVBG.newAccount is not callable")
	}
...
func (b *bridge) OpenWallet(call jsre.Call) (goja.Value, error) {
	...
	openWallet, callable := goja.AssertFunction(getJVBG(call.VM).Get("openWallet"))
	if !callable {
		return nil, fmt.Errorf("jVBG.openWallet is not callable")
	}
...
// UnlockAccount is a wrapper around the personal.unlockAccount RPC mVBGod that
// uses a non-echoing password prompt to acquire the passphrase and executes the
// original RPC mVBGod (saved in jVBG.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call jsre.Call) (goja.Value, error) {
	...
	unlockAccount, callable := goja.AssertFunction(getJVBG(call.VM).Get("unlockAccount"))
	if !callable {
		return nil, fmt.Errorf("jVBG.unlockAccount is not callable")
	}
	return unlockAccount(goja.Null(), account, passwd, duration)
}

// Sign is a wrapper around the personal.sign RPC mVBGod that uses a non-echoing password
// prompt to acquire the passphrase and executes the original RPC mVBGod (saved in
// jVBG.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call jsre.Call) (goja.Value, error) {
	...
	// Send the request to the backend and return
	sign, callable := goja.AssertFunction(getJVBG(call.VM).Get("sign"))
	if !callable {
		return nil, fmt.Errorf("jVBG.sign is not callable")
	}
...
func (b *bridge) SleepBlocks(call jsre.Call) (goja.Value, error) {
	...
	if err := b.client.Call(&lastNumber, "VBG_blockNumber"); err != nil {
		return nil, err
	}
	for time.Now().Before(deadline) {
		var number hexutil.Uint64
		if err := b.client.Call(&number, "VBG_blockNumber"); err != nil {
			return nil, err
		}
...
// Send implements the web3 provider "send" mVBGod.
func (b *bridge) Send(call jsre.Call) (goja.Value, error) {
	...
	for _, req := range reqs {
		...
		if err = b.client.Call(&result, req.MVBGod, req.Params...); err == nil {
			if result == nil {
