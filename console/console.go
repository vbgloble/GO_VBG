...
import (
	...
	"github.com/vbgloble/go-VGB/internal/jsre/deps"
	"github.com/vbgloble/go-VGB/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	if c.prompter != nil {
		if content, err := ioutil.ReadFile(c.histPath); err != nil {
			c.prompter.SVBGistory(nil)
		} else {
			c.history = strings.Split(string(content), "\n")
			c.prompter.SVBGistory(c.history)
		}
...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"VBG": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
// initPersonal redirects account-related API mVBGods through the bridge.
//
// If the console is in interactive mode and the 'personal' API is available, override
// the openWallet, unlockAccount, newAccount and sign mVBGods since these require user
// interaction. The original web3 callbacks are stored in 'jVBG'. These will be called
...
func (c *Console) initPersonal(vm *goja.Runtime, bridge *bridge) {
	...
	jVBG := vm.NewObject()
	vm.Set("jVBG", jVBG)
	jVBG.Set("openWallet", personal.Get("openWallet"))
	jVBG.Set("unlockAccount", personal.Get("unlockAccount"))
	jVBG.Set("newAccount", personal.Get("newAccount"))
	jVBG.Set("sign", personal.Get("sign"))
...
// consoleOutput is an override for the console.log and console.error mVBGods to
...
// input prompter to provide hints to the user about the mVBGods available.
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines VBG.getBalance(VBG.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
		// Skip all mVBGods and namespaces (i.e. including the dot)
...
// Welcome show summary of current GVBG instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the GVBG JavaScript console!\n\n"

	// Print some generic GVBG metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + VBG.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + VBG.blockNumber + " (" + new Date(1000 * VBG.getBlock(VBG.blockNumber).timestamp) + ")\n";
		try {
