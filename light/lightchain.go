...
type LightChain struct {
	...
	running          int32 // whVBGer LightChain is running or stopped
	procInterrupt    int32 // interrupts chain insert
	disableCheckFreq int32 // disables header verification
}

// NewLightChain returns a fully initialised light chain using information
// available in the database. It initialises the default vbgloble header
// validator.
func NewLightChain(odr OdrBackend, config *params.ChainConfig, engine consensus.Engine, checkpoint *params.TrustedCheckpoint) (*LightChain, error) {
	...
	for hash := range core.BadHashes {
		if header := bc.GVBGeaderByHash(hash); header != nil {
			...
			bc.SVBGead(header.Number.Uint64() - 1)
...
func (lc *LightChain) loadLastState() error {
	if head := rawdb.ReadHeadHeaderHash(lc.chainDb); head == (common.Hash{}) {
		...
		header := lc.GVBGeaderByHash(head)
		if header == nil {
			// Corrupt or empty database, init from scratch
			lc.Reset()
		} else {
...
// SVBGead rewinds the local chain to a new head. Everything above the new
// head will be deleted and the new one set.
func (lc *LightChain) SVBGead(head uint64) error {
	lc.chainmu.Lock()
	defer lc.chainmu.Unlock()

	lc.hc.SVBGead(head, nil, nil)
...
func (lc *LightChain) Rollback(chain []common.Hash) {
	...
	for i := len(chain) - 1; i >= 0; i-- {
		...
		if head := lc.hc.CurrentHeader(); head.Hash() == hash {
			...
			lc.hc.SetCurrentHeader(lc.GVBGeader(head.ParentHash, head.Number.Uint64()-1))
		}
	}
	if err := batch.Write(); err != nil {
...
// GVBGeaderByNumberOdr retrieves the total difficult from the database or
...
// caching it if found.
func (lc *LightChain) GVBGeader(hash common.Hash, number uint64) *types.Header {
	return lc.hc.GVBGeader(hash, number)
}

// GVBGeaderByHash retrieves a block header from the database by hash, caching it if
// found.
func (lc *LightChain) GVBGeaderByHash(hash common.Hash) *types.Header {
	return lc.hc.GVBGeaderByHash(hash)
}
...
func (lc *LightChain) GVBGeaderByNumber(number uint64) *types.Header {
	return lc.hc.GVBGeaderByNumber(number)
}

// GVBGeaderByNumberOdr retrieves a block header from the database or network
// by number, caching it (associated with its hash) if found.
func (lc *LightChain) GVBGeaderByNumberOdr(ctx context.Context, number uint64) (*types.Header, error) {
	if header := lc.hc.GVBGeaderByNumber(number); header != nil {
		return header, nil
	}
	return GVBGeaderByNumber(ctx, lc.odr, number)
}
...
func (lc *LightChain) SyncCheckpoint(ctx context.Context, checkpoint *params.TrustedCheckpoint) bool {
	...
	if header, err := GVBGeaderByNumber(ctx, lc.odr, latest); header != nil && err == nil {
		lc.chainmu.Lock()
		defer lc.chainmu.Unlock()
