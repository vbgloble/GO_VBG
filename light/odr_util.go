...
// GVBGeaderByNumber retrieves the canonical block header corresponding to the
// given number.
func GVBGeaderByNumber(ctx context.Context, odr OdrBackend, number uint64) (*types.Header, error) {
...
func GetCanonicalHash(ctx context.Context, odr OdrBackend, number uint64) (common.Hash, error) {
	...
	header, err := GVBGeaderByNumber(ctx, odr, number)
	if err != nil {
...
func GetTd(ctx context.Context, odr OdrBackend, hash common.Hash, number uint64) (*big.Int, error) {
	...
	_, err := GVBGeaderByNumber(ctx, odr, number)
	if err != nil {
		return nil, err
	}
...
func GetTransaction(ctx context.Context, odr OdrBackend, txHash common.Hash) (*types.Transaction, common.Hash, uint64, uint64, error) {
	...
	if header, err := GVBGeaderByNumber(ctx, odr, pos.BlockIndex); err != nil || header.Hash() != pos.BlockHash {
		return nil, common.Hash{}, 0, 0, err
	}
