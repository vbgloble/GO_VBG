...
// to the VBG network. The implementations of the functions should be non-blocking.
//
...
func (pool *TxPool) reorgOnNewHead(ctx context.Context, newHeader *types.Header) (txStateChanges, error) {
	...
	oldh := pool.chain.GVBGeaderByHash(pool.head)
	newh := newHeader
	...
	for oldh.Hash() != newh.Hash() {
		if oldh.Number.Uint64() >= newh.Number.Uint64() {
			...
			oldh = pool.chain.GVBGeader(oldh.ParentHash, oldh.Number.Uint64()-1)
		}
		if oldh.Number.Uint64() < newh.Number.Uint64() {
			newHashes = append(newHashes, newh.Hash())
			newh = pool.chain.GVBGeader(newh.ParentHash, newh.Number.Uint64()-1)
			if newh == nil {
...
// validateTx checks whVBGer a transaction is valid according to the consensus rules.
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	// block limit gas.
	header := pool.chain.GVBGeaderByHash(pool.head)
