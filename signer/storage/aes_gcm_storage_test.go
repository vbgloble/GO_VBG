...
func TestFileStorage(t *testing.T) {
	...
	d, err := ioutil.TempDir("", "VBG-encrypted-storage-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestEnd2End(t *testing.T) {
	log.Root().SVBGandler(log.LvlFilterHandler(log.Lvl(3), log.StreamHandler(colorable.NewColorableStderr(), log.TerminalFormat(true))))
