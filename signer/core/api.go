...
import (
	...
	"github.com/vbgloble/go-VGB/signer/storage"
)
...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, mVBGodSelector *string) (*VBGapi.SignTransactionResult, error)
	...
	SignGnosisSafeTx(ctx context.Context, signerAddress common.MixedcaseAddress, gnosisTx GnosisSafeTx, mVBGodSelector *string) (*GnosisSafeTx, error)
}

// UIClientAPI specifies what mVBGod a UI needs to implement to be able to be used as a
// UI for the signer
type UIClientAPI interface {
	...
	// This mVBGod can be used by a UI to keep track of e.g. how much has been sent to a particular recipient.
	OnApprovedTx(tx VBGapi.SignTransactionResult)
...
// Validator defines the mVBGods required to validate a transaction against some
// sanity defaults as well as any underlying 4byte mVBGod database.
//
...
// newAccount is the internal mVBGod to create a new account. It should be used
...
func (api *SignerAPI) lookupOrQueryPassword(address common.Address, title, prompt string) (string, error) {
	...
	if err != nil {
		...
		// which could leak the password if it was malformed json or somVBGing
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, mVBGodSelector *string) (*VBGapi.SignTransactionResult, error) {
	var (
		err    error
		result SignTxResponse
	)
	msgs, err := api.validator.ValidateTransaction(mVBGodSelector, &args)
	if err != nil {
		return nil, err
	}
	...
	response := VBGapi.SignTransactionResult{Raw: rlpdata, Tx: signedTx}
...
func (api *SignerAPI) SignGnosisSafeTx(ctx context.Context, signerAddress common.MixedcaseAddress, gnosisTx GnosisSafeTx, mVBGodSelector *string) (*GnosisSafeTx, error) {
...
// Returns the external api version. This mVBGod does not require user acceptance. Available mVBGods are
