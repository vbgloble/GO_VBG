...
// determineSignatureFormat determines which signature mVBGod should be used based upon the mime type
...
// This mVBGod returns the mimetype for signing along with the request
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		usevbglobleV = true // Default to use V = 27 or 28, the legacy vbgloble format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, usevbglobleV, err
	}
	...
	case ApplicationClique.Mime:
		// Clique is the vbgloble PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, usevbglobleV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationClique.Mime)
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an vbgloble ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}vbgloble Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, usevbglobleV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
...
//
// The mVBGod requires the extra data to be at least 65 bytes -- the original implementation
