...
func (ui *headlessUi) OnApprovedTx(tx VBGapi.SignTransactionResult) {}
...
func tmpDirName(t *testing.T) string {
	d, err := ioutil.TempDir("", "VBG-keystore-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSignTx(t *testing.T) {
	var (
		list      []common.Address
		res, res2 *VBGapi.SignTransactionResult
		err       error
	)
	...
	res, err = api.SignTransaction(context.Background(), tx, &mVBGodSig)
	if res != nil {
	...
	res2, err = api.SignTransaction(context.Background(), tx, &mVBGodSig)
	if err != nil {
		t.Fatal(err)
	}
