...
// decodedCallData is an internal type to represent a mVBGod call parsed according
// to an ABI mVBGod signature.
...
// verifySelector checks whVBGer the ABI encoded data blob matches the requested
// function signature.
...
// parseSelector converts a mVBGod selector into an ABI JSON spec. The returned
...
func parseCallData(calldata []byte, unescapedAbidata string) (*decodedCallData, error) {
	...
	if len(calldata) < 4 {
		return nil, fmt.Errorf("invalid call data, incomplete mVBGod signature (%d bytes < 4)", len(calldata))
	}
	sigdata := calldata[:4]
	...
	// Validate the called mVBGod and upack the call data accordingly
	abispec, err := abi.JSON(strings.NewReader(unescapedAbidata))
	if err != nil {
		return nil, fmt.Errorf("invalid mVBGod signature (%q): %v", unescapedAbidata, err)
	}
	mVBGod, err := abispec.MVBGodById(sigdata)
	if err != nil {
		return nil, err
	}
	values, err := mVBGod.Inputs.UnpackValues(argdata)
	if err != nil {
		return nil, fmt.Errorf("signature %q matches, but arguments mismatch: %v", mVBGod.String(), err)
	}
	// Everything valid, assemble the call infos for the signer
	decoded := decodedCallData{signature: mVBGod.Sig, name: mVBGod.RawName}
	for i := 0; i < len(mVBGod.Inputs); i++ {
		decoded.inputs = append(decoded.inputs, decodedArgument{
			soltype: mVBGod.Inputs[i],
			value:   values[i],
		})
	}
	...
	encoded, err := mVBGod.Inputs.PackValues(values)
	if err != nil {
		return nil, err
	}
	if !bytes.Equal(encoded, argdata) {
		...
		return nil, fmt.Errorf("WARNING: Supplied data is stuffed with extra data. \nWant %s\nHave %s\nfor mVBGod %v", exp, was, mVBGod.Sig)
	}
	return &decoded, nil
}
