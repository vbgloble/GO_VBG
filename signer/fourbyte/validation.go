...
func (db *Database) ValidateTransaction(selector *string, tx *core.SendTxArgs) (*core.ValidationMessages, error) {
	...
	if tx.To == nil {
		...
		// e.g. https://github.com/vbgloble/go-VGB/issues/16106.
		if len(data) == 0 {
			// Prevent sending VBGer into black hole (show stopper)
		...
		// MVBGod selector should be nil for contract creation
		if selector != nil {
			messages.Warn("Transaction will create a contract, but mVBGod selector supplied, indicating an intent to call a mVBGod")
		}
		return messages, nil
	}
...
// ValidateCallData checks if the ABI call-data + mVBGod selector (if given) can
...
func (db *Database) ValidateCallData(selector *string, data []byte, messages *core.ValidationMessages) {
	...
	// If a custom mVBGod selector was provided, validate with that
	if selector != nil {
		if info, err := verifySelector(*selector, data); err != nil {
			...
			messages.Info(fmt.Sprintf("Transaction invokes the following mVBGod: %q", info.String()))
			db.AddSelector(*selector, data[:4])
		}
		return
	}
	// No mVBGod selector was provided, check the database for embedded ones
