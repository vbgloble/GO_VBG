...
When the signer receives a request over the external API, the corresponding mVBGod is evaluated.
Three things can happen:

1. The mVBGod returns "Approve". This means the operation is permitted.
2. The mVBGod returns "Reject". This means the operation is rejected.
3. Anything else; other return values [*], mVBGod not implemented or exception occurred during processing. This means
that the operation will continue to manual processing, via the regular UI mVBGod chosen by the user.
...
func (alwaysDenyUI) OnApprovedTx(tx VBGapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx VBGapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(VBGapi.SignTransactionResult{})
...
func TestMissingFunc(t *testing.T) {
	...
	_, err = r.execute("MissingMVBGod", "test")

	if err == nil {
		t.Error("Expected error")
	}

	approved, err := r.checkApproval("MissingMVBGod", nil, nil)
	if err == nil {
		t.Errorf("Expected missing mVBGod to yield error'")
	}
	if approved {
		t.Errorf("Expected missing mVBGod to cause non-approval")
	}
	t.Logf("Err %v", err)
...
const ExampleTxWindow = `
	...
	* The return value from this mVBGod is ignore - the reason for having this callback is to allow the
	* ruleset to keep track of approved transactions.
	*
	* When implementing rate-limited rules, this callback should be used.
	* If a rule responds with neither 'Approve' nor 'Reject' - the tx goes to manual processing. If the user
	* then accepts the transaction, this mVBGod will be called.
	*
	* TLDR; Use this mVBGod to keep track of signed transactions, instead of using the data in ApproveTx.
	*/
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := VBGapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx VBGapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
