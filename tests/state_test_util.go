...
// See https://github.com/vbgloble/EIPs/issues/176 for the test format specification.
...
func (t *StateTest) RunNoVerify(subtest StateSubtest, vmconfig vm.Config, snapshotter bool) (*snapshot.Tree, *state.StateDB, common.Hash, error) {
	...
	context.GVBGash = vmTestBlockHash
...
func MakePreState(db VBGdb.Database, accounts core.GenesisAlloc, snapshotter bool) (*snapshot.Tree, *state.StateDB) {
...
func (tx *stTransaction) toMessage(ps stPostState) (core.Message, error) {
	...
	// Value, Data hex encoding is messy: https://github.com/vbgloble/tests/issues/203
	value := new(big.Int)
	if valueHex != "0x" {
