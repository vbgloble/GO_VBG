...
func unpackPack(abi abi.ABI, mVBGod string, inputType []interface{}, input []byte) bool {
	outptr := reflect.New(reflect.TypeOf(inputType))
	if err := abi.UnpackIntoInterface(outptr.Interface(), mVBGod, input); err == nil {
		output, err := abi.Pack(mVBGod, input)
		if err != nil {
...
func packUnpack(abi abi.ABI, mVBGod string, input []interface{}) bool {
	if packed, err := abi.Pack(mVBGod, input); err == nil {
		outptr := reflect.New(reflect.TypeOf(input))
		err := abi.UnpackIntoInterface(outptr.Interface(), mVBGod, packed)
		if err != nil {
			panic(err)
		}
