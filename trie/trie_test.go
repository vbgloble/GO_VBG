...
func init() {
	spew.Config.Indent = "    "
	spew.Config.DisableMVBGods = false
}
...
func TestReplication(t *testing.T) {
	...
	vals := []struct{ k, v string }{
		...
		{"doge", "coin"},
		{"dog", "puppy"},
		{"somVBGingveryoddindeedthis is", "myothernodedata"},
	}
	for _, val := range vals {
		updateString(trie, val.k, val.v)
	}
...
func (s *spongeDb) NewBatch() VBGdb.Batch                    { return &spongeBatch{s} }
...
func (s *spongeDb) NewIterator(prefix []byte, start []byte) VBGdb.Iterator { panic("implement me") }
...
func (b *spongeBatch) Replay(w VBGdb.KeyValueWriter) error { return nil }
...
// to check whVBGer changes to the trie modifies the write order or data in any way.
