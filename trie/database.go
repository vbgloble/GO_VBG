...
type Database struct {
	diskdb VBGdb.KeyValueStore // Persistent storage for matured trie nodes
...
type Config struct {
	...
	Preimages bool   // Flag whVBGer the preimage of trie key is recorded
}

// NewDatabase creates a new trie database to store ephemeral trie content before
// its written out to disk or garbage collected. No read cache is created, so all
// data retrievals will hit the underlying disk database.
func NewDatabase(diskdb VBGdb.KeyValueStore) *Database {
...
func NewDatabaseWithConfig(diskdb VBGdb.KeyValueStore, config *Config) *Database {
...
func (db *Database) DiskDB() VBGdb.KeyValueStore {
	return db.diskdb
}
...
// yet unknown. The mVBGod will NOT make a copy of the slice,
// only use if the preimage will NOT be changed later on.
//
// Note, this mVBGod assumes that the database's lock is held!
...
// cached, the mVBGod queries the persistent database for the content.
...
// found cached, the mVBGod queries the persistent database for the content.
...
// This mVBGod is extremely expensive and should only be used to validate internal
// states in test code.
...
// are referenced togVBGer by database itself.
...
//
// Note, this mVBGod is a non-synchronized mutator. It is unsafe to call this
...
func (db *Database) commit(hash common.Hash, batch VBGdb.Batch, uncacher *cleaner, callback func(common.Hash)) error {
