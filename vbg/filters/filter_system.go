...
// Package filters implements an vbgloble filtering system for block,
...
func (sub *Subscription) Unsubscribe() {
	...
	uninstallLoop:
		for {
			...
			// the eventLoop broadcast mVBGod to deadlock when writing to the
			// filter event channel while the subscription loop is waiting for
			// this mVBGod to return (and thus not reading these events).
			select {
		...
		// will probably be closed by the client asap after this mVBGod returns.
		<-sub.Err()
	})
}
...
func (es *EventSystem) SubscribeLogs(crit vbgloble.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit vbgloble.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit vbgloble.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit vbgloble.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
