...
func init() {
	// log.Root().SVBGandler(log.LvlFilterHandler(log.LvlTrace, log.StreamHandler(os.Stderr, log.TerminalFormat(false))))
}
...
func TestForkIDSplit(t *testing.T) {
	var (
		...
		VBGNoFork, _  = NewProtocolManager(configNoFork, nil, downloader.FullSync, 1, new(event.TypeMux), &testTxPool{pool: make(map[common.Hash]*types.Transaction)}, engine, chainNoFork, dbNoFork, 1, nil)
		VBGProFork, _ = NewProtocolManager(configProFork, nil, downloader.FullSync, 1, new(event.TypeMux), &testTxPool{pool: make(map[common.Hash]*types.Transaction)}, engine, chainProFork, dbProFork, 1, nil)
	)
	VBGNoFork.Start(1000)
	VBGProFork.Start(1000)
	...
	go func() { errc <- VBGNoFork.handle(peerProFork) }()
	go func() { errc <- VBGProFork.handle(peerNoFork) }()
