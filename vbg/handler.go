...
import (
	...
	"github.com/vbgloble/go-VGB/VBG/fetcher"
...
type ProtocolManager struct {
	...
	fastSync  uint32 // Flag whVBGer fast sync is enabled (gets disabled if we already have blocks)
	acceptTxs uint32 // Flag whVBGer we're considered synchronised (enables transaction processing)
...
// NewProtocolManager returns a new vbgloble sub protocol manager. The vbgloble sub protocol manages peers capable
// with the vbgloble network.
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb VBGdb.Database, cacheLimit int, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
	...
	inserter := func(blocks types.Blocks) (int, error) {
		...
		// out a way yet where nodes can decide unilaterally whVBGer the network is new
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing vbgloble peer", "peer", id)

	// Unregister the peer from the downloader and vbgloble peer set
...
func (pm *ProtocolManager) Start(maxPeers int) {
	...
	go pm.txsyncLoop64() // TODO(karalabe): Legacy initial tx echange, drop with VBG/64.
}
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("vbgloble protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an VBG peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("vbgloble peer connected", "name", p.Name())

	// Execute the vbgloble handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkID, pm.forkFilter); err != nil {
		p.Log().Debug("vbgloble handshake failed", "err", err)
		return err
	}

	// Register the peer locally
	if err := pm.peers.Register(p, pm.removePeer); err != nil {
		p.Log().Error("vbgloble peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("vbgloble message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == GetBlockHeadersMsg:
		...
		for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit && len(headers) < downloader.MaxHeaderFetch {
			...
			if hashMode {
				if first {
					first = false
					origin = pm.blockchain.GVBGeaderByHash(query.Origin.Hash)
					if origin != nil {
						query.Origin.Number = origin.Number.Uint64()
					}
				} else {
					origin = pm.blockchain.GVBGeader(query.Origin.Hash, query.Origin.Number)
				}
			} else {
				origin = pm.blockchain.GVBGeaderByNumber(query.Origin.Number)
			}
			if origin == nil {
				break
			}
			...
			case hashMode && !query.Reverse:
				...
				if next <= current {
					...
					if header := pm.blockchain.GVBGeaderByNumber(next); header != nil {
	...
	case p.version >= VBG63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= VBG63 && msg.Code == NodeDataMsg:
	...
	case p.version >= VBG63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
		...
		for bytes < softResponseLimit && len(receipts) < downloader.MaxReceiptFetch {
			...
			if results == nil {
				if header := pm.blockchain.GVBGeaderByHash(hash); header == nil || header.ReceiptHash != types.EmptyRootHash {
					continue
				}
			}
	...
	case p.version >= VBG63 && msg.Code == ReceiptsMsg:
	...
	case msg.Code == NewBlockMsg:
		...
		if _, td := p.Head(); trueTD.Cmp(td) > 0 {
			p.SVBGead(trueHead, trueTD)
			pm.chainSync.handlePeerEvent(p)
		}

	case msg.Code == NewPooledTransactionHashesMsg && p.version >= VBG65:
	...
	case msg.Code == GetPooledTransactionsMsg && p.version >= VBG65:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case msg.Code == TransactionMsg || (msg.Code == PooledTransactionsMsg && p.version >= VBG65):
...
// NodeInfo represents a short summary of the vbgloble sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // vbgloble network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
