...
type peerConnection struct {
	...
	statVBGroughput   float64 // Number of node data pieces measured to be retrievable per second
	...
	version int        // VBG protocol version number to switch strategies
	log     log.Logger // Contextual logger to add extra infos to peer logs
	lock    sync.RWMutex
}

// LightPeer encapsulates the mVBGods required to synchronise with a remote light peer.
...
// Peer encapsulates the mVBGods required to synchronise with a remote full peer.
type Peer interface {
	LightPeer
...
// lightPeerWrapper wraps a LightPeer struct, stubbing out the Peer-only mVBGods.
...
// SVBGeadersIdle sets the peer to idle, allowing it to execute new header retrieval
// requests. Its estimated header retrieval throughput is updated with that measured
// just now.
func (p *peerConnection) SVBGeadersIdle(delivered int, deliveryTime time.Time) {
...
func (p *peerConnection) SetNodeDataIdle(delivered int, deliveryTime time.Time) {
	p.setIdle(deliveryTime.Sub(p.stateStarted), delivered, &p.statVBGroughput, &p.stateIdle)
}
...
func (p *peerConnection) setIdle(elapsed time.Duration, delivered int, throughput *float64, idle *int32) {
	...
	*throughput = (1-measurementImpact)*(*throughput) + measurementImpact*measured
	...
	p.log.Trace("Peer throughput measurements updated",
		...
		"rps", p.receiptThroughput, "sps", p.statVBGroughput,
...
func (p *peerConnection) NodeDataCapacity(targetRTT time.Duration) int {
	...
	return int(math.Min(1+math.Max(1, p.statVBGroughput*float64(targetRTT)/float64(time.Second)), float64(MaxStateFetch)))
}
...
// Lacks retrieves whVBGer the hash of a blockchain item is on the peers lacking
// list (i.e. whVBGer we know that the peer does not have it).
...
//
// The mVBGod also sets the starting throughput values of the new peer to the
...
func (ps *peerSet) Register(p *peerConnection) error {
	...
	if len(ps.peers) > 0 {
		p.headerThroughput, p.blockThroughput, p.receiptThroughput, p.statVBGroughput = 0, 0, 0, 0
		...
		for _, peer := range ps.peers {
			...
			p.statVBGroughput += peer.statVBGroughput
			peer.lock.RUnlock()
		}
		p.headerThroughput /= float64(len(ps.peers))
		p.blockThroughput /= float64(len(ps.peers))
		p.receiptThroughput /= float64(len(ps.peers))
		p.statVBGroughput /= float64(len(ps.peers))
	}
	ps.peers[p.id] = p
	ps.lock.Unlock()
