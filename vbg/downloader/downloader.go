...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [VBG/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    VBGdb.Database  // Database to state sync into (and deduplicate via)
	...
	// Channels
	headerCh      chan dataPack        // [VBG/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [VBG/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [VBG/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [VBG/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [VBG/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [VBG/62] Channel to feed the header processor new tasks
	...
	stateCh        chan dataPack // [VBG/63] Channel receiving inbound node state data
	...
	// Testing hooks
	syncInitHook     func(uint64, uint64)  // MVBGod to call upon initiating a new sync run
	bodyFetchHook    func([]*types.Header) // MVBGod to call upon starting a block body fetch
	receiptFetchHook func([]*types.Header) // MVBGod to call upon starting a receipt fetch
	chainInsertHook  func([]*fetchResult)  // MVBGod to call upon inserting a chain of blocks (possibly in multiple invocations)
}
...
type LightChain interface {
	...
	// GVBGeaderByHash retrieves a header from the local chain.
	GVBGeaderByHash(common.Hash) *types.Header
	...
	// SVBGead rewinds the local chain to a new head.
	SVBGead(uint64) error
}
...
func New(checkpoint uint64, stateDb VBGdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() vbgloble.SyncProgress {
...
// Synchronising returns whVBGer the downloader is currently retrieving blocks.
...
func (d *Downloader) Synchronise(id string, head common.Hash, td *big.Int, mode SyncMode) error {
	...
	if errors.Is(err, errInvalidChain) || errors.Is(err, errBadPeer) || errors.Is(err, errTimeout) ||
		...
		if d.dropPeer == nil {
			// The dropPeer mVBGod is nil when `--copydb` is used for a local copy.
...
// checks fail an error will be returned. This mVBGod is synchronous
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "VBG", p.version, "head", hash, "td", td, "mode", mode)
	...
	if mode == FastSync {
		...
		if origin+1 < frozen {
			if err := d.lightchain.SVBGead(origin + 1); err != nil {
				return err
			}
		}
	}
...
// not wait for the running download goroutines to finish. This mVBGod should be
...
func (d *Downloader) findAncestor(p *peerConnection, remoteHeader *types.Header) (uint64, error) {
	...
	if mode == LightSync {
		...
		if d.genesis == 0 {
			...
			for header != nil {
				...
				header = d.lightchain.GVBGeaderByHash(header.ParentHash)
			}
		}
	...
	for finished := false; !finished; {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave somVBGing valid
			headers := packet.(*headerPack).headers
			if len(headers) == 0 {
	...
	for start+1 < end {
		...
		for arrived := false; !arrived; {
			...
			case packet := <-d.headerCh:
				...
				header := d.lightchain.GVBGeaderByHash(h) // Independent of sync mode, header surely exists
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64) error {
	...
	pivoting := false           // WhVBGer the next request is pivot verification
...
//
// The mVBGod returns the entire filled skeleton and also the number of headers
...
func (d *Downloader) fillHeaderSkeleton(from uint64, skeleton []*types.Header) ([]*types.Header, int, error) {
	...
	var (
		...
		setIdle  = func(p *peerConnection, accepted int, deliveryTime time.Time) {
			p.SVBGeadersIdle(accepted, deliveryTime)
		}
	)
...
// types, this mVBGod is used by each for data gathering and is instrumented with
...
//  - expire:      task callback mVBGod to abort requests that took too long and return the faulty peers (traffic shaping)
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered somVBGing completely else than requested (usually
...
func (d *Downloader) processHeaders(origin uint64, td *big.Int) error {
	...
	defer func() {
		if rollback > 0 {
			...
			if err := d.lightchain.SVBGead(rollback - 1); err != nil { // -1 to target the parent of the first uncertain block
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have somVBGing)
				// R: Nothing to give
				if mode != LightSync {
				...
				// peer gave us somVBGing useful, we're already happy/progressed (above check).
			...
			gotHeaders = true
			for len(headers) > 0 {
				// Terminate if somVBGing failed in between processing chunks
				select {
				case <-d.cancelCh:
					rollbackErr = errCanceled
					return errCanceled
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The InsertChain mVBGod in blockchain.go will sometimes return an out-of-bounds index,
			// when it needs to preprocess blocks to import a sidechain.
			// The importer will put togVBGer a new list of blocks to import, which is a superset
