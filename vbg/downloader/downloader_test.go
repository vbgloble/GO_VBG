...
type downloadTester struct {
	...
	stateDb VBGdb.Database // Database used by the tester for syncing from peers
	peerDb  VBGdb.Database // Database of the peers containing all data
...
func (dl *downloadTester) HasHeader(hash common.Hash, number uint64) bool {
	return dl.GVBGeaderByHash(hash) != nil
}
...
// GVBGeader retrieves a header from the testers canonical chain.
func (dl *downloadTester) GVBGeaderByHash(hash common.Hash) *types.Header {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
	return dl.gVBGeaderByHash(hash)
}

// gVBGeaderByHash returns the header if found either within ancients or own blocks)
// This mVBGod assumes that the caller holds at least the read-lock (dl.lock)
func (dl *downloadTester) gVBGeaderByHash(hash common.Hash) *types.Header {
...
func (dl *downloadTester) InsertHeaderChain(headers []*types.Header, checkFreq int) (i int, err error) {
	...
	if dl.gVBGeaderByHash(headers[0].ParentHash) == nil {
	...
	for i, header := range headers {
		hash := hashes[i]
		if dl.gVBGeaderByHash(hash) != nil {
			continue
		}
		if dl.gVBGeaderByHash(header.ParentHash) == nil {
...
func (dl *downloadTester) InsertChain(blocks types.Blocks) (i int, err error) {
	...
	for i, block := range blocks {
		...
		if hdr := dl.gVBGeaderByHash(block.Hash()); hdr == nil {
...
func (dl *downloadTester) SVBGead(head uint64) error {
	dl.lock.Lock()
	defer dl.lock.Unlock()
...
// RequestBodies constructs a getBlockBodies mVBGod associated with a particular
...
// RequestReceipts constructs a getReceipts mVBGod associated with a particular
// peer in the download tester. The returned function can be used to retrieve
...
// RequestNodeData constructs a getNodeData mVBGod associated with a particular
// peer in the download tester. The returned function can be used to retrieve
...
func assertOwnChain(t *testing.T, tester *downloadTester, length int) {
	// Mark this mVBGod as a helper to report errors at callsite, not in here
	t.Helper()
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", vbgloble.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", vbgloble.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", vbgloble.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", vbgloble.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want vbgloble.SyncProgress) {
	// Mark this mVBGod as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", vbgloble.SyncProgress{
