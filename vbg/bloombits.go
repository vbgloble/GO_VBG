...
const (
	// bloomServicVBGreads is the number of goroutines used globally by an vbgloble
	// instance to service bloombits lookups for all running filters.
	bloomServicVBGreads = 16
...
func (VBG *vbgloble) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServicVBGreads; i++ {
		go func() {
			for {
				select {
				case <-VBG.closeBloomHandler:
					return

				case request := <-VBG.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(VBG.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(VBG.chainDb, task.Bit, section, head); err == nil {
...
// for the vbgloble header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size    uint64               // section size to generate bloombits for
	db      VBGdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db VBGdb.Database, size, confirms uint64) *core.ChainIndexer {
