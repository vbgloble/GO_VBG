...
const (
	...
	//   - In VBG protocol, the softResponseLimit is 2MB. Nowadays according to
	//     VBGerscan the average transaction size is around 200B, so in theory
...
var (
	txAnnounceInMeter          = metrics.NewRegisteredMeter("VBG/fetcher/transaction/announces/in", nil)
	txAnnounceKnownMeter       = metrics.NewRegisteredMeter("VBG/fetcher/transaction/announces/known", nil)
	txAnnounceUnderpricedMeter = metrics.NewRegisteredMeter("VBG/fetcher/transaction/announces/underpriced", nil)
	txAnnounceDOSMeter         = metrics.NewRegisteredMeter("VBG/fetcher/transaction/announces/dos", nil)

	txBroadcastInMeter          = metrics.NewRegisteredMeter("VBG/fetcher/transaction/broadcasts/in", nil)
	txBroadcastKnownMeter       = metrics.NewRegisteredMeter("VBG/fetcher/transaction/broadcasts/known", nil)
	txBroadcastUnderpricedMeter = metrics.NewRegisteredMeter("VBG/fetcher/transaction/broadcasts/underpriced", nil)
	txBroadcastOtherRejectMeter = metrics.NewRegisteredMeter("VBG/fetcher/transaction/broadcasts/otherreject", nil)

	txRequestOutMeter     = metrics.NewRegisteredMeter("VBG/fetcher/transaction/request/out", nil)
	txRequestFailMeter    = metrics.NewRegisteredMeter("VBG/fetcher/transaction/request/fail", nil)
	txRequestDoneMeter    = metrics.NewRegisteredMeter("VBG/fetcher/transaction/request/done", nil)
	txRequestTimeoutMeter = metrics.NewRegisteredMeter("VBG/fetcher/transaction/request/timeout", nil)

	txReplyInMeter          = metrics.NewRegisteredMeter("VBG/fetcher/transaction/replies/in", nil)
	txReplyKnownMeter       = metrics.NewRegisteredMeter("VBG/fetcher/transaction/replies/known", nil)
	txReplyUnderpricedMeter = metrics.NewRegisteredMeter("VBG/fetcher/transaction/replies/underpriced", nil)
	txReplyOtherRejectMeter = metrics.NewRegisteredMeter("VBG/fetcher/transaction/replies/otherreject", nil)

	txFetcherWaitingPeers   = metrics.NewRegisteredGauge("VBG/fetcher/transaction/waiting/peers", nil)
	txFetcherWaitingHashes  = metrics.NewRegisteredGauge("VBG/fetcher/transaction/waiting/hashes", nil)
	txFetcherQueueingPeers  = metrics.NewRegisteredGauge("VBG/fetcher/transaction/queueing/peers", nil)
	txFetcherQueueingHashes = metrics.NewRegisteredGauge("VBG/fetcher/transaction/queueing/hashes", nil)
	txFetcherFetchingPeers  = metrics.NewRegisteredGauge("VBG/fetcher/transaction/fetching/peers", nil)
	txFetcherFetchingHashes = metrics.NewRegisteredGauge("VBG/fetcher/transaction/fetching/hashes", nil)
)
...
type txDelivery struct {
	...
	direct bool          // WhVBGer this is a direct reply or a broadcast
}
...
// NewTxFetcherForTests is a testing mVBGod to mock out the realtime clock with
...
// and the fetcher. This mVBGod may be called by both transaction broadcasts and
...
func (f *TxFetcher) loop() {
	...
	for {
		...
		case ann := <-f.notify:
			...
			// the probability of somVBGing arriving between this call and the pre-
			...
			// If this peer is new and announced somVBGing already queued, maybe
		...
		case <-timeoutTrigger:
			...
			// No idea if we scheduled somVBGing or not, trigger the timer if needed
		...
		case delivery := <-f.cleanup:
			...
			if delivery.direct {
				...
				// Make sure somVBGing was pending, nuke it
				...
				// SomVBGing was delivered, try to rechedule requests
		...
		// Loop did somVBGing, ping the step notifier if needed (tests)
		if f.step != nil {
			f.step <- struct{}{}
		}
	}
...
//
// The mVBGod has a granularity of 'gatherSlack', since there's not much point in
...
// a few ms earlier.
//
// This mVBGod is a bit "flaky" "by design". In theory the timeout timer only ever
...
// be set again since len(request) > 0 => somVBGing's running.
func (f *TxFetcher) rescheduleTimeout(timer *mclock.Timer, trigger chan struct{}) {
	...
	for _, req := range f.requests {
		// If this request already timed out, skip it altogVBGer
		if req.hashes == nil {
			continue
		}
...
// rotateStrings rotates the contents of a slice by n steps. This mVBGod is only
...
// sortHashes sorts a slice of hashes. This mVBGod is only used in tests in order
...
// rotateHashes rotates the contents of a slice by n steps. This mVBGod is only
// used in tests to simulate random map iteration but keep it deterministic.
