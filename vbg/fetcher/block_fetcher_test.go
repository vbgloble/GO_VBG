...
func makeChain(n int, seed byte, parent *types.Block) ([]common.Hash, map[common.Hash]*types.Block) {
	blocks, _ := core.GenerateChain(params.TestChainConfig, parent, VBGash.NewFaker(), testdb, n, func(i int, block *core.BlockGen) {
...
func newTester(light bool) *fetcherTester {
	...
	tester.fetcher = NewBlockFetcher(light, tester.gVBGeader, tester.getBlock, tester.verifyHeader, tester.broadcastBlock, tester.chainHeight, tester.insertHeaders, tester.insertChain, tester.dropPeer)
	tester.fetcher.Start()

	return tester
}

// gVBGeader retrieves a header from the tester's block chain.
func (f *fetcherTester) gVBGeader(hash common.Hash) *types.Header {
	f.lock.RLock()
...
func testPendingDeduplication(t *testing.T, light bool) {
	...
	if light {
		checkNonExist = func() bool {
			return tester.gVBGeader(hashes[0]) == nil
		}
	}
