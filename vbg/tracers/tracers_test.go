...
import (
	...
	"github.com/vbgloble/go-VGB/tests"
)

// To generate a new callTracer test, copy paste the makeTest mVBGod below into
// a GVBG console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = VBG.getBlock(VBG.getTransaction(tx).blockHash);
  var genesis = VBG.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.VBG.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  VBG.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
