...
// PeerInfo represents a short summary of the vbgloble sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // vbgloble protocol version negotiated
...
// SVBGead updates the head hash and total difficulty of the peer.
func (p *peer) SVBGead(hash common.Hash, td *big.Int) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
//
// This mVBGod is legacy support for initial transaction exchange in VBG/64 and
// prior. For VBG/65 and higher use SendPooledTransactionHashes.
...
// in its transaction hash set for future reference.
//
// This mVBGod is a helper used by the async transaction sender. Don't call it
...
//
// This mVBGod is a helper used by the async transaction announcer. Don't call it
// directly as the queueing (memory) and transmission (bandwidth) costs should
// not be managed directly.
...
//
// Note, the mVBGod assumes the hashes are correct and correspond to the list of
...
// Handshake executes the VBG protocol handshake, negotiating version number,
...
func (p *peer) Handshake(network uint64, td *big.Int, head common.Hash, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter) error {
	...
	go func() {
		...
		default:
			panic(fmt.Sprintf("unsupported VBG protocol version: %d", p.version))
		}
	}()
	go func() {
		switch {
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("VBG/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the vbgloble sub-protocol.
type peerSet struct {
