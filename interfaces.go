...
// Package vbgloble defines interfaces for interacting with vbgloble.
package vbgloble
...
// NotFound is returned by API mVBGods if the requested item does not exist.
...
// ChainReader provides access to the blockchain. The mVBGods in this interface access raw
...
type ChainReader interface {
	...
	// This mVBGod subscribes to notifications about changes of the head block of
	// the canonical chain.
...
type CallMsg struct {
	...
	Data     []byte          // input data, usually an ABI-encoded contract mVBGod invocation
}

// A ContractCaller provides contract calls, essentially transactions that are executed by
// the EVM but not mined into the blockchain. ContractCall is a low-level mVBGod to
...
type FilterQuery struct {
	BlockHash *common.Hash     // used by VBG_getLogs, return logs only from block with this hash
...
// TransactionSender wraps transaction sending. The SendTransaction mVBGod injects a
// signed transaction into the pending transaction pool for execution. If the transaction
// was a contract creation, the TransactionReceipt mVBGod can be used to retrieve the
