...
type (
	...
	// function description, along with whVBGer it should run inside an operation callback.
	NodeStateMachine struct {
		started, closed     bool
		lock                sync.Mutex
		clock               mclock.Clock
		db                  VBGdb.KeyValueStore
		dbNodeKey           []byte
...
// flagOp implements binary flag operations and also checks whVBGer the operands belong to the same setup
...
func NewNodeStateMachine(db VBGdb.KeyValueStore, dbKey []byte, clock mclock.Clock, setup *Setup) *NodeStateMachine {
	if setup.flags == nil {
...
// stateMask checks whVBGer the set of flags belongs to the same setup and returns its internal bit mask
...
// fieldIndex checks whVBGer the field belongs to the same setup and returns its internal index
...
// checkStarted checks whVBGer the state machine has already been started and panics otherwise.
...
// opCheck checks whVBGer an operation is active
