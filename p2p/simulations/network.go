...
// starting nodes and connecting them togVBGer.
//
...
// Connect connects two nodes togVBGer by calling the "admin_addPeer" RPC
// mVBGod on the "one" node so that it connects to the "other" node
...
// mVBGod on the "one" node so that it disconnects from the "other" node
...
// GetRandomNode returns a random node on the network, regardless of whVBGer it is running or not
...
// NOTE:
// it also checks whVBGer there has been recent attempt to connect the peers
...
func (net *Network) Shutdown() {
	for _, node := range net.Nodes {
		...
		// If the node has the close mVBGod, call it.
...
type Node struct {
	...
	// up tracks whVBGer or not the node is running
	up   bool
	upMu *sync.RWMutex
}
...
// Up returns whVBGer the node is currently up (online)
func (n *Node) Up() bool {
	n.upMu.RLock()
...
func (n *Node) UnmarshalJSON(raw []byte) error {
	...
	// Ticket: https://github.com/VBGersphere/go-VGB/issues/1177
	var node struct {
		Config *adapters.NodeConfig `json:"config,omitempty"`
		Up     bool                 `json:"up"`
	}
...
type Conn struct {
	...
	// Up tracks whVBGer or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whVBGer both nodes are currently up
