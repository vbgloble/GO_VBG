...
import (
	...
	"github.com/vbgloble/go-VGB/p2p/discover/v4wire"
...
const (
	...
	ntpFailurVBGreshold = 32               // Continuous timeouts after which to check NTP
...
type replyMatcher struct {
	...
	// reply was acceptable. The second return value indicates whVBGer the callback should
...
type reply struct {
	...
	ip   net.IP
	data v4wire.Packet
	// loop indicates whVBGer there was
...
// whVBGer any matcher considered the packet acceptable.
...
func (t *UDPv4) loop() {
	...
	for {
		...
		case r := <-t.gotreply:
			var matched bool // whVBGer any replyMatcher considered the reply acceptable.
			for el := plist.Front(); el != nil; el = el.Next() {
		...
		case now := <-timeout.C:
			...
			if contTimeouts > ntpFailurVBGreshold {
...
func (t *UDPv4) wrapPacket(p v4wire.Packet) *packVBGandlerV4 {
	var h packVBGandlerV4
	h.Packet = p
	switch p.(type) {
	case *v4wire.Ping:
...
// packVBGandlerV4 wraps a packet with handler functions.
type packVBGandlerV4 struct {
	v4wire.Packet
	senderKey *ecdsa.PublicKey // used for ping

	// preverify checks whVBGer the packet is valid and should be handled at all.
	preverify func(p *packVBGandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error
	// handle handles the packet.
	handle func(req *packVBGandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte)
}

// PING/v4

func (t *UDPv4) verifyPing(h *packVBGandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
...
func (t *UDPv4) handlePing(h *packVBGandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte) {
	req := h.Packet.(*v4wire.Ping)
...
func (t *UDPv4) verifyPong(h *packVBGandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
...
func (t *UDPv4) verifyFindnode(h *packVBGandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
...
func (t *UDPv4) handleFindnode(h *packVBGandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte) {
	req := h.Packet.(*v4wire.Findnode)
...
func (t *UDPv4) verifyNeighbors(h *packVBGandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
...
func (t *UDPv4) verifyENRRequest(h *packVBGandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
...
func (t *UDPv4) handleENRRequest(h *packVBGandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte) {
...
func (t *UDPv4) verifyENRResponse(h *packVBGandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
	if !t.handleReply(fromID, from.IP, h.Packet) {
		return errUnsolicitedReply
	}
	return nil
