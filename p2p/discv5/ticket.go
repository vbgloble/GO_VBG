...
func (s *ticketStore) adjustWithTicket(now mclock.AbsTime, targVBGash common.Hash, t *ticket) {
	for i, topic := range t.topics {
		if tt, ok := s.radius[topic]; ok {
			tt.adjustWithTicket(now, targVBGash, ticketRef{t, i})
		}
	}
}
...
func (r *topicRadius) adjustWithTicket(now mclock.AbsTime, targVBGash common.Hash, t ticketRef) {
	...
	r.adjust(now, targVBGash, t.t.node.sha, inside)
}

func (r *topicRadius) adjust(now mclock.AbsTime, targVBGash, addrHash common.Hash, inside float64) {
	...
	delete(r.buckets[bucket].lookupSent, targVBGash)
}
