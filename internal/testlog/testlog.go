...
// t.Logf(). All mVBGods in between logger.Trace, logger.Debug, etc. are marked as test
...
func Logger(t *testing.T, level log.Lvl) log.Logger {
	...
	l.l.SVBGandler(log.LvlFilterHandler(level, l.h))
	return l
}
...
func (l *logger) GVBGandler() log.Handler {
	return l.l.GVBGandler()
}

func (l *logger) SVBGandler(h log.Handler) {
	l.l.SVBGandler(h)
}
