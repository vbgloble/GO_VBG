...
type TestCmd struct {
	// For total convenience, all testing mVBGods are available.
	*testing.T
...
// reexec init function for that name (e.g. "gVBG-test" in cmd/gVBG/run_test.go)
...
// This mVBGod can also be called from an expect template, e.g.:
//
//     gVBG.expect(`Passphrase: {{.InputLine "password"}}`)
