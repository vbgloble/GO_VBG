...
func (c *lesCommons) setupOracle(node *node.Node, genesis common.Hash, VBGconfig *VBG.Config) *checkpointoracle.CheckpointOracle {
	config := VBGconfig.CheckpointOracle
	if config == nil {
	...
	client := VBGclient.NewClient(rpcClient)
	oracle.Start(client)
