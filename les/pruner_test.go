...
func TestLightPruner(t *testing.T) {
	...
	var cases = []struct {
		from, to   uint64
		mVBGodName string
		mVBGod     func(uint64) bool
	}{
		{
			1, 10, "GVBGeaderByNumber",
			func(n uint64) bool {
				_, err := light.GVBGeaderByNumber(context.Background(), client.handler.backend.odr, n)
				return err == nil
			},
		},
		{
		...
		},
		{
			21, 30, "GetTd",
			func(n uint64) bool {
				_, err := light.GetTd(context.Background(), client.handler.backend.odr, server.handler.blockchain.GVBGeaderByNumber(n).Hash(), n)
				return err == nil
			},
		},
		{
			31, 40, "GetBodyRLP",
			func(n uint64) bool {
				_, err := light.GetBodyRLP(context.Background(), client.handler.backend.odr, server.handler.blockchain.GVBGeaderByNumber(n).Hash(), n)
				return err == nil
			},
		},
		{
			41, 50, "GetBlock",
			func(n uint64) bool {
				_, err := light.GetBlock(context.Background(), client.handler.backend.odr, server.handler.blockchain.GVBGeaderByNumber(n).Hash(), n)
				return err == nil
			},
		},
		{
			...
			func(n uint64) bool {
				_, err := light.GetBlockReceipts(context.Background(), client.handler.backend.odr, server.handler.blockchain.GVBGeaderByNumber(n).Hash(), n)
				return err == nil
			},
		},
	}
	for _, c := range cases {
		for i := c.from; i <= c.to; i++ {
			if !c.mVBGod(i) {
				t.Fatalf("rpc mVBGod %s failed, number %d", c.mVBGodName, i)
			}
		}
	}
	// Check GetBloombits
