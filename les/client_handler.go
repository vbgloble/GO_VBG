...
func newClientHandler(ulcServers []string, ulcFraction int, checkpoint *params.TrustedCheckpoint, backend *Lightvbgloble) *clientHandler {
...
func (h *clientHandler) handle(p *serverPeer) error {
	...
	if err := h.backend.peers.register(p); err != nil {
		p.Log().Error("Light vbgloble peer registration failed", "err", err)
		return err
	}
...
func (d *downloaderPeerNotify) registerPeer(p *serverPeer) {
	...
	h.downloader.RegisterLightPeer(p.id, VBGVersion, pc)
}
