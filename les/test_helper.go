...
import (
	...
	"github.com/vbgloble/go-VGB/les/checkpointoracle"
	"github.com/vbgloble/go-VGB/les/flowcontrol"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			// bankUser transfers some VBGer to user1
		...
		case 1:
			...
			// bankUser transfers more VBGer to user1
		...
		case 2:
			// bankUser transfer some VBGer to signer
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
...
func testIndexers(db VBGdb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
	var indexers [3]*core.ChainIndexer
	indexers[0] = light.NewChtIndexer(db, odr, config.ChtSize, config.ChtConfirms, disablePruning)
	indexers[1] = VBG.NewBloomIndexer(db, config.BloomSize, config.BloomConfirms)
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db VBGdb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
		evmux  = new(event.TypeMux)
		engine = VBGash.NewFaker()
		gspec  = core.Genesis{
			Config:   params.AllVBGashProtocolChanges,
	...
	client := &Lightvbgloble{
		lesCommons: lesCommons{
			genesis:     genesis.Hash(),
			config:      &VBG.Config{LightPeers: 100, NetworkId: NetworkId},
			chainConfig: params.AllVBGashProtocolChanges,
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db VBGdb.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllVBGashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
