...
import (
	...
	"github.com/vbgloble/go-VGB/les/utils"
...
type peerCommons struct {
	*p2p.Peer
	...
	frozen       uint32    // Flag whVBGer the peer is frozen.
...
// canQueue returns an indicator whVBGer the peer can queue an operation.
...
func (p *peerCommons) Info() *VBG.PeerInfo {
	return &VBG.PeerInfo{
		Version:    p.version,
		Difficulty: p.Td(),
...
type serverPeer struct {
	...
	// Status fields
	trusted                 bool   // The flag whVBGer the server is selected as trusted server.
	onlyAnnounce            bool   // The flag whVBGer the server sends announcement only.
	...
	// Test callback hooks
	hasBlockHook func(common.Hash, uint64, bool) bool // Used to determine whVBGer the server has the specified block.
}
...
func (p *serverPeer) requestHelperTrieProofs(reqID uint64, reqs []HelperTrieReq) error {
	...
	return p.sendRequest(GVBGelperTrieProofsMsg, reqID, reqs, len(reqs))
}
	...
	return p.handshake(td, head, headNum, genesis, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local vbgloble node is running in archive mode, advertise ourselves we have
...
// participating in the Light vbgloble sub-protocol.
