...
func testGetBlockHeaders(t *testing.T, protocol int) {
	...
	for i, tt := range tests {
		...
		for _, hash := range tt.expect {
			headers = append(headers, bc.GVBGeaderByHash(hash))
		}
...
func testGetStaleCode(t *testing.T, protocol int) {
	...
	check := func(number uint64, expected [][]byte) {
		req := &CodeReq{
			BHash:  bc.GVBGeaderByNumber(number).Hash(),
			AccKey: crypto.Keccak256(testContractAddr[:]),
		}
...
func testGetStaleProof(t *testing.T, protocol int) {
	...
	check := func(number uint64, wantOK bool) {
		var (
			header  = bc.GVBGeaderByNumber(number)
...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	header := bc.GVBGeaderByNumber(config.ChtSize - 1)
	...
	root := light.GetChtRoot(server.db, 0, bc.GVBGeaderByNumber(config.ChtSize-1).Hash())
	...
	// Send the proof request and verify the response
	sendRequest(server.peer.app, GVBGelperTrieProofsMsg, 42, requestsV2)
...
func testGetBloombitsProofs(t *testing.T, protocol int) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		root := light.GetBloomTrieRoot(server.db, 0, bc.GVBGeaderByNumber(config.BloomTrieSize-1).Hash())
		trie, _ := trie.New(root, trie.NewDatabase(rawdb.NewTable(server.db, light.BloomTrieTablePrefix)))
		trie.Prove(key, 0, &proofs.Proofs)

		// Send the proof request and verify the response
		sendRequest(server.peer.app, GVBGelperTrieProofsMsg, 42, requests)
...
func testTransactionStatus(t *testing.T, protocol int) {
	...
	gchain, _ := core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), VBGash.NewFaker(), server.db, 1, func(i int, block *core.BlockGen) {
		block.AddTx(tx1)
		block.AddTx(tx2)
	})
	...
	gchain, _ = core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), VBGash.NewFaker(), server.db, 2, func(i int, block *core.BlockGen) {})
	if _, err := chain.InsertChain(gchain); err != nil {
		panic(err)
	}
