...
// the VBG.BlockFetcher as the underlying fetcher but adding more additional
...
func newLightFetcher(chain *light.LightChain, engine consensus.Engine, peers *serverPeerSet, ulc *ulc, chaindb VBGdb.Database, reqDist *requestDistributor, syncFn func(p *serverPeer)) *lightFetcher {
	...
	f := &lightFetcher{
		...
		chain:       chain,
		reqDist:     reqDist,
		fetcher:     fetcher.NewBlockFetcher(true, chain.GVBGeaderByHash, nil, validator, nil, heighter, inserter, nil, dropper),
...
func (f *lightFetcher) mainloop() {
	...
	var (
		...
		syncing      bool        // Indicator whVBGer the client is syncing
	...
	// trustedHeader returns an indicator whVBGer the header is regarded as
		...
		case ev := <-headCh:
			...
			f.forEachPeer(func(id enode.ID, p *fetcherPeer) bool {
				...
				for _, anno := range removed {
					if header := f.chain.GVBGeaderByHash(anno.data.Hash); header != nil {
		...
		case origin := <-f.syncDone:
			...
			if ulc {
				...
				for head.Number.Cmp(ancestor.Number) > 0 {
					...
					head = f.chain.GVBGeader(head.ParentHash, number-1)
				}
				if len(untrusted) > 0 {
...
//
// Note, we rely on the underlying VBG/fetcher to retrieve and validate the
// response, so that we have to obey the rule of VBG/fetcher which only accepts
