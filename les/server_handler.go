...
import (
	...
	lps "github.com/vbgloble/go-VGB/les/lespay/server"
	...
	"github.com/vbgloble/go-VGB/p2p/nodestate"
...
const (
	...
	VBGVersion        = 63              // equivalent VBG version for the downloader
...
type serverHandler struct {
	...
	synced  func() bool    // Callback function used to determine whVBGer local node is synced.

	// Testing fields
	addTxsSync bool
}

func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb VBGdb.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	p.Log().Debug("Light vbgloble peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), h.server); err != nil {
		p.Log().Debug("Light vbgloble handshake failed", "err", err)
		return err
	}
	...
	if cap, err := h.server.clientPool.connect(p); cap != p.fcParams.MinRecharge || err != nil {
		p.Log().Debug("Light vbgloble peer rejected", "err", errFullClientPool)
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light vbgloble response", "err", err)
			return err
		default:
		}
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light vbgloble message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light vbgloble message arrived", "code", msg.Code, "bytes", msg.Size)
	...
	// accept returns an indicator whVBGer the request can be served.
		...
		if accept(req.ReqID, query.Amount, MaxHeaderFetch) {
			...
			go func() {
				...
				for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit {
					...
					if hashMode {
						if first {
							origin = h.blockchain.GVBGeaderByHash(query.Origin.Hash)
							if origin != nil {
								query.Origin.Number = origin.Number.Uint64()
							}
						} else {
							origin = h.blockchain.GVBGeader(query.Origin.Hash, query.Origin.Number)
						}
					} else {
						origin = h.blockchain.GVBGeaderByNumber(query.Origin.Number)
					}
					if origin == nil {
						break
					}
					...
					case hashMode && !query.Reverse:
						...
						if next <= current {
							...
							if header := h.blockchain.GVBGeaderByNumber(next); header != nil {
		...
		if accept(req.ReqID, uint64(reqCnt), MaxCodeFetch) {
			...
			go func() {
				...
				for i, request := range req.Reqs {
					...
					header := h.blockchain.GVBGeaderByHash(request.BHash)
					if header == nil {
		...
		if accept(req.ReqID, uint64(reqCnt), MaxReceiptFetch) {
			...
			go func() {
				...
				for i, hash := range req.Hashes {
					...
					if results == nil {
						if header := h.blockchain.GVBGeaderByHash(hash); header == nil || header.ReceiptHash != types.EmptyRootHash {
							p.bumpInvalid()
							continue
						}
					}
			...
			go func() {
				...
				for i, request := range req.Reqs {
					...
					if request.BHash != lastBHash {
						...
						if header = h.blockchain.GVBGeaderByHash(request.BHash); header == nil {
			...
			go func() {
				...
				for i, request := range req.Reqs {
					...
					if auxTrie == nil || request.Type != lastType || request.TrieIdx != lastIdx {
						...
						var prefix string
						if root, prefix = h.gVBGelperTrie(request.Type, request.TrieIdx); root != (common.Hash{}) {
...
// gVBGelperTrie returns the post-processed trie root for the given trie ID and section index
func (h *serverHandler) gVBGelperTrie(typ uint, index uint64) (common.Hash, string) {
	switch typ {
	case htCanonical:
