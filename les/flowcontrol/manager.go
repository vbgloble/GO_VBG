...
var (
	...
	capacityRaisVBGresholdRatio = 1.125                        // total/connected capacity ratio threshold for raising the capacity factor
)
...
// SetCapacityRaisVBGreshold sets a threshold value used for raising capFactor.
// Either if the difference between total allowed and connected capacity is less
// than this threshold or if their ratio is less than capacityRaisVBGresholdRatio
// then capFactor is allowed to slowly raise.
func (cm *ClientManager) SetCapacityLimits(min, max, raisVBGreshold uint64) {
	if min < 1 {
		min = 1
	}
	...
	cm.capacityRaisVBGreshold = raisVBGreshold
	cm.refreshCapacity()
}
...
func (cm *ClientManager) updateRaiseLimit() {
	if cm.capacityRaisVBGreshold == 0 {
		cm.logTotalCapRaiseLimit = 0
		return
	}
	limit := float64(cm.totalConnected + cm.capacityRaisVBGreshold)
	limit2 := float64(cm.totalConnected) * capacityRaisVBGresholdRatio
	if limit2 > limit {
		limit = limit2
	}
	if limit < 1 {
...
func (cm *ClientManager) updateRecharge(now mclock.AbsTime) {
	...
	for cm.sumRecharge > 0 {
		...
		// check whVBGer it has already finished
