...
import (
	...
	"github.com/vbgloble/go-VGB/p2p/simulations"
	"github.com/vbgloble/go-VGB/p2p/simulations/adapters"
...
func TestMain(m *testing.M) {
	...
	log.Root().SVBGandler(log.LvlFilterHandler(log.Lvl(*loglevel), log.StreamHandler(colorable.NewColorableStderr(), log.TerminalFormat(true))))
...
// similar requests at the maximum allowed rate and the test verifies whVBGer the
...
func testCapacityAPI(t *testing.T, clientCount int) {
	...
	for !testSim(t, 1, clientCount, []string{testServerDataDir}, nil, func(ctx context.Context, net *simulations.Network, servers []*simulations.Node, clients []*simulations.Node) bool {
		...
		headNum, headHash := gVBGead(ctx, t, serverRpcClient)
		...
		for i, client := range clients {
			...
			for {
				...
				num, hash := gVBGead(ctx, t, clientRpcClients[i])
...
func gVBGead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64, common.Hash) {
	res := make(map[string]interface{})
	if err := client.CallContext(ctx, &res, "VBG_getBlockByNumber", "latest", false); err != nil {
...
func testRequest(ctx context.Context, t *testing.T, client *rpc.Client) bool {
	...
	defer cancel()
	err := client.CallContext(c, &res, "VBG_getBalance", addr, "latest")
	if err != nil {
...
func newLesServerService(ctx *adapters.ServiceContext, stack *node.Node) (node.Lifecycle, error) {
	...
	vbgloble, err := VBG.New(stack, &config)
	if err != nil {
		return nil, err
	}
	_, err = NewLesServer(stack, vbgloble, &config)
	if err != nil {
		return nil, err
	}
	return vbgloble, nil
